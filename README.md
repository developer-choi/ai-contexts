...
구글 문서에 쌓아둔 개발 필기를 AI가 바로 써먹을 수 있는 프롬프트로 살리고 있습니다. "A 케이스에서는 B로 구현해야 한다"라고 적어둔 메모는 나중에 돌아보는 정도지만, 이걸 프롬프트로 만들면 AI에게 곧바로 전달해서 코드 리뷰나 구현 가이드로 쓸 수 있습니다.

---

## 핵심 기능: Audit Skill — 프롬프트 검증 시스템
...
`npm run update`는 `deploy/` 폴더를 `~/.claude/`로 복사합니다.
