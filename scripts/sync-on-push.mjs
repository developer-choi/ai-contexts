#!/usr/bin/env node
// pre-push (.githooks) — master(메인 브랜치)를 push할 때 sync:environment + sync:system + sync:local-system을 함께 돌린다.
...
function main() {
  ...
  const steps = [
    ["sync:environment", path.join("scripts", "environment", "sync-environment.mjs")],
