...
function main() {
  ...
  // hook 본체 = 폴더 내 다른 .mjs가 import하지 않는 .mjs. import되는 것(hook-utils 등)은 라이브러리라 제외.
  ...
  for (const f of hookFiles) {
    ...
    // ESM 정적 import의 로컬 상대 경로(확장자 .mjs 포함)를 라이브러리로 본다.
  ...
  const dangling = baseFiles.filter((f) => !hookFiles.includes(f));
  ...
  check(!codex.some((h) => h.file === 'post-enterworktree-install.mjs'),
