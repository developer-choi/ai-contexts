...
// 2. 스쿼시 베이스는 origin/master (로컬 master는 메인 워크트리 checked-out이라 옮기지 않음)
const masterBase = `${REMOTE}/master`;
...
// 4. 동일 → origin/master 위로 스쿼시
const count = parseInt(git(['rev-list', '--count', `${masterBase}..${BRANCH}`]), 10);
if (count <= 1) {
  console.log(`이미 ${masterBase} 위 ${count}커밋입니다. 스쿼시 생략.`);
  process.exit(0);
}
console.log(`${masterBase} 위 ${count}커밋을 1개로 스쿼시합니다.`);
mutate(['reset', '--soft', masterBase]);
mutate(['commit', '-m', 'chore(backlog): 백로그 커밋 정리']);

if (DRY) {
  console.log('\n[dry-run] 스쿼시 후 force-push 안내가 출력됩니다.');
} else {
