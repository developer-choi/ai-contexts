...
- 프롬프트를 추가하기 전에, 그 규칙이 프롬프트가 아닌 다른 메커니즘으로 강제할 수 있는지 먼저 확인한다. 린트로 잡을 수 있으면 린트 규칙으로, 훅으로 자동화할 수 있으면 훅으로 구현한다.
...
- 내용을 추가·수정·삭제할 때 `map.md` 상단의 "역할" 섹션을 확인하고, 코드 예제로 보여줄 수 있는 내용은 AC가 아닌 MP `docs/best-practices-map.md`에 둔다
