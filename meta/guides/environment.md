...
- `scripts/hooks/check-md-size.mjs`를 `~/.ai-contexts/check-md-size.mjs`로 복사하고, `--global` pre-commit 훅(`hook.ai-contexts-md-size.*`)으로 멱등하게 등록합니다. 커밋하는 레포의 프롬프트·스킬 md(`CLAUDE.md`·`local/`·`deploy/` 하위)를 전수로 재서 「주제의 축척은 크기가 정한다」의 선을 넘은 문서를 경고합니다 — 커밋을 막지는 않습니다.
  ...
  - 이미 선을 넘은 문서는 등재해 조용히 둡니다(`node ~/.ai-contexts/check-md-size.mjs --write-baseline`). 첫날 수십 건을 매 커밋 쏟으면 그 경고를 안 보게 되고, 그러면 새로 넘는 파일도 함께 묻힙니다. 등재분은 더 커질 때만 걸리고, 선 아래로 내려오면 등재를 걷으라고 알립니다. 등재 목록은 크기 × 닿는 곳 순으로 적혀 그대로 다이어트 대상 목록이 됩니다.
