...
- 예외: `skills`의 각 SKILL.md는 복사 후 배포본 형태로 고칩니다 — frontmatter에 `name`이 없으면 폴더명을 주입하고(Antigravity는 `name`이 있어야 스킬을 인식합니다), 본문 맨 앞에 그 스킬이 배포된 절대 경로를 한 줄 심습니다(Antigravity는 스킬 위치를 모델에게 알려주지 않아 본문의 상대 경로가 안 풀립니다).
