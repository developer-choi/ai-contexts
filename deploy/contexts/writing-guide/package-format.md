...
- `rejected`: 사용자가 이 문서에서 거부한 표현을 `"표현 원문" (결 이름)` 한 줄씩 담습니다. 결 이름은 채점기가 쓰는 분류 이름(자기 산출물 과장·추상 압축어·외래어 음차·극적 수식어 등, `score.mjs --rules`로 전부 볼 수 있습니다)을 재사용합니다. 표현 원문만 피하고 같은 결의 다른 표현을 새로 짓는 것을 막기 위해 결까지 적습니다.
