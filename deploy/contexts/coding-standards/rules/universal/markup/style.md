...
## 구현 주석 지양
구현 상세를 설명하는 주석은 작성하지 않습니다. 코드는 그 자체로 의도가 드러나야 합니다.
- 변수명과 함수명을 명확하게 지어 로직을 설명하세요.
- 비즈니스 로직이나 구현 상세를 설명하는 주석은 금지합니다. (단, 라이브러리 제약이나 특수한 환경 등으로 인한 우회 로직 등 '이유'가 반드시 필요한 경우에만 예외적으로 아주 짧게 작성할 수 있습니다.)