...
## 셀프 리뷰 기준
- 자신의 산출물(코드, 문서, 계획)을 리뷰할 때, 대기업 시니어 개발자가 기술 면접을 진행하는 관점으로 본다.
- "리뷰했습니다"만 말하지 않는다 — 무엇을 리뷰했고 어떤 판단을 내렸는지 포함한다.
...
- 문서·프롬프트·설정 파일(.md, .json 등)만 수정하는 작업은 묻지 않는다.
- 사용자가 불필요하다고 답해야 /workflow 없이 진행한다.
...
- 서로 다른 컴포넌트/유틸/프롬프트를 하나의 커밋에 섞지 않는다
- 마이그레이션/설계 문서와 실제 코드/프롬프트 변경을 같은 커밋에 섞지 않는다
- squash 시 같은 종류끼리 묶는다 (마이그레이션 문서끼리, 구현 커밋끼리)

### 내 작업 외 변경은 커밋하지 않는다
- 동일 프로젝트에서 병렬 세션이 동시 작업 중일 수 있다. 다른 세션의 변경을 내 커밋에 섞으면 그 세션이 판단해야 할 수정을 내가 대신 커밋하게 된다.
- `git add` 시 내가 수정한 파일만 이름으로 명시하여 스테이징한다 (`git add -A`, `git add .` 금지).
- `git status`/`git diff`에서 내가 건드리지 않은 변경이 보이면, 사용자에게 알리고 별도 처리한다. 내 커밋에 섞어 넣지 않는다.
