...
//
// 왜 산문이 아니라 훅인가: [가리키는 줄은 목적지에 닿게 쓴다](prompt-standards/sources-and-links.md#가리키는-줄은-목적지에-닿게-쓴다)가
...
// 보므로 이건 아무도 안 본다. scw 특화 체크리스트의 before 블록 넷이 그렇게 낡아 있었다(그 파일은
// 이후 rules-as-code.md로 편입되며 사라졌다).
//
...
// 산문이 레포를 부르는 약어 → 폴더 이름. [프로젝트 약어](deploy/rules/global.md#프로젝트-약어)의 사본이다 —
...
//
// **이 규칙으로 만든 앵커를 되푸는 쪽이 레포 밖에 있다.** 훅이 옮기라고 시킨 앵커 링크를 PP
// `local/contexts/recruitment/scripts/site-usage.mjs`가 같은 규칙으로 풀어 「인용된 절」을 센다.
// 규칙을 바꾸면 그쪽도 함께 본다 — 어긋나면 앵커를 못 풀어 인용을 조용히 0으로 세고, 회차 조사
// 범위를 정하는 「떠왔는데 안 쓴 절」이 100%로 나온다. 레포가 갈려 결속 등록부에 못 묶는다 —
// 양쪽 주석이 서로를 가리킨다.
function slug(text) {
  return text
    .replace(/[`*]/g, "")
...
function formatSoft(found) {
  ...
  if (found.legacy.length) {
    ...
    lines.push(
      ...
      "",
      "  · 대상은 이 파일에 남은 옛 표기 전부입니다. 누가 언제 썼는지는 중요하지 않습니다.",
