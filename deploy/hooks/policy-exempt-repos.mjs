...
// git 정책을 통째로 면제하는 레포. 사용자가 자기 것만 담아 자유롭게 쓰는 곳이라,
// 머지·푸시·reset을 "사용자 결정"으로 묶어둘 이유가 없다.
//
// 가르는 축은 소유가 아니라 이 목록에 올랐는지다. 사용자 소유 레포는 이 둘 말고도 있고
// 그쪽은 정책을 유지하므로, "개인 레포라서 면제"로 설명하면 목록 밖 레포를 틀리게 가른다.
//
...
export function allInPolicyExemptRepos(invocations, fallbackCwd) {
  if (invocations.length === 0) return false;
  return invocations.every((inv) => isPolicyExemptRepo(normalizeCwd(inv.cwd) || fallbackCwd));
}
