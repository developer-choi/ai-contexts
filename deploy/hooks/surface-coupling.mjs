...
try {
  ...
  addContext(
    `[짝꿍 동기화] 지금 수정하려는 ${rel} 에는 같이 수정해야 할 짝꿍이 등록돼 있다:\n` +
      blocks.join("\n") +
      `\n이번 수정을 끝낸 뒤 위 짝꿍 파일을 직접 열어 대조한다. **이번에 바꾼 부분만 보지 않는다** ` +
      `— 앞선 변경이 반영 안 된 채 남아 있을 수 있으므로, 짝꿍끼리 대응하는 항목을 처음부터 훑는다. ` +
      `이번 변경과 무관하게 어긋난 것을 발견하면 임의로 고치지 말고 사용자에게 알린다. ` +
      `어긋난 게 없으면 넘어간다. 도구는 차단하지 않는다.`,
    "PreToolUse",
  );
} catch (_e) {
