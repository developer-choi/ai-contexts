...
//
// 목적은 한 줄이고, 「왜 이 스킬이 필요한가」(존재 근거)는 별개 절이다 (deploy/contexts/
// prompt-standards/skill-structure.md 「목적」). 그런데 목적 초안을 쓸 때마다 "이게 왜 중요한가"를 한 문단 더 붙이는
// 쏠림이 반복됐다 — 규칙은 그 파일에 있고 초안은 다른 자리에서 쓰이니, 쓰는 순간에
