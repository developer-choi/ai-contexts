...
- 다듬기 지시: "writing-guide 규칙·사례 적용해서 본문 다듬기. **frontmatter는 절대 건드리지 마.** 사실 검증·수정은 안 함. 이모지를 추가하지 않는다. 원본에 이모지가 있어도 제거하지 않는다. frontmatter `rendering_env`가 `plain-text`면 마크다운 문법(백틱, `#` 헤딩, `**`, `[]()` 등)을 본문에서 제거한다. frontmatter `placeholder_policy`에 따라 본문 플레이스홀더를 처리한다 — 명시 없으면 `keep`."
