...
- **사용자가 직접 쓰거나 불러넣은 구절을 그 출처(발화 원문·사용자 편집 자리)와 함께 적어 둔다.** 다음 단계에서 리뷰어에게 함께 넘긴다 — 안 넘기면 리뷰어는 그것이 사용자의 말인 줄 몰라 매 라운드 반려로 되돌린다.
...
- **기계 층 (0까지 반복, 메인)**: `../pre-exit/augmentations/score.mjs`를 직접 실행해 검증한다(`type`이 `resume-*`면 `--resume` 추가). 출력의 **기계 적발 합계**가 0이 될 때까지 직접 재수정 → 재실행을 반복한다. 반복 상한은 글로벌 규칙 「검증은 수렴할 때까지 반복」을 따르고, 상한에서 잔존하면 멈추고 잔여 위반과 함께 보고한다. **리뷰어를 부르기 전에 닫는다.**
  - 합계에 안 들어가고 "눈으로 확인"으로 표시된 항목(문맥 확인 필요, 습니다체 휴리스틱, 개수 선언 뒤 리스트 없음, 「~화」 한자어, `placeholder_policy: keep`의 placeholder 잔존)은 0-강제 대상이 아니다 — 출력된 후보를 직접 읽고 실제 위반만 고른다. **여기서 0을 만들려 들지 않는다** — 정상 문장이 걸릴 수 있는 자리라, 0을 밀면 멀쩡한 문장을 깎게 된다.
...
- **형제 문서 양식 대조**: 발행처에 같은 계열 문서가 이미 있으면 그중 하나를 열어 본문 밖 양식(목차 블록·상하단 네비게이션·머리말 배치)을 대조하고 맞춘다. 그 양식은 발행 주소가 있어야 쓸 수 있어 원고에는 존재할 수 없으므로, 원고와 발행본을 맞대는 검사로는 영영 안 걸린다. 형제에 없는 것은 임의로 붙이지 않는다.
