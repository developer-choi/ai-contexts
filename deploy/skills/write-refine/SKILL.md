...
- `rejected`가 있으면 그 표현과 **같은 결**의 표현을 이번 수정에 넣지 않는다(「4. 보고·사용자 피드백」의 「거부한 표현 재투입 금지」 판단 근거). 표현 원문만 피하고 같은 결을 새로 짓는 것도 재투입이다.
...
- **반객관 층 (0까지 반복, 최대 5회)**: 자기판단 대신 `../pre-exit/augmentations/score.mjs`를 직접 실행해 검증한다(`type`이 `resume-*`면 `--resume` 추가). 출력의 **금지어(tone.md 한자어·추상압축어·외래어음차·자기과장어·극적수식어) · em/en dash(인용블록 밖) · 빈 섹션** 위반 건수 합이 0이 될 때까지 직접 재수정 → 재실행을 반복한다. frontmatter 제외·`key_message`/`rejected` 무시는 스크립트가 처리하므로 따로 챙기지 않는다.
  - **습니다체 종결**은 스크립트도 "눈으로 확인 필요"로 표시하는 항목이라 0-강제 대상에서 제외한다 — 출력된 후보를 직접 읽고 실제 위반만 고른다(이력서는 명사형 허용).
...
- **거부한 표현 재투입 금지**: 사용자가 한 번 거부한 결의 표현(추상 압축어 등)을 다음 수정에 또 넣지 않는다. 거부가 나올 때마다 frontmatter `rejected`에 `"표현 원문" (결 이름)`으로 append한다 — 이 세션의 기억은 다음 재실행에 남지 않으므로, 파일에 적어야 규정이 다음 세션에서도 발동한다.
  - 다른 글에도 반복될 선호로 보이면 pre-exit 절차대로 tone.md·examples에 심자고 제안한다. `rejected`는 이 문서 한정 기록이다.
