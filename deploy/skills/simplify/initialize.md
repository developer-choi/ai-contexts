...
- **로컬 git 정체성은 이 커밋 전에 설정한다** — 그래야 initial commit부터 개인 계정으로 author된다. 글로벌 git 정체성이 **개인 계정(`forworkchoe@gmail.com`)이 아니면** 로컬 config를 개인 계정으로 설정한다.
- 기본 브랜치는 `main`으로 둔다.

### (선택) 개인 GitHub 계정으로 push

푸시가 필요하면 인증이 **개인 계정**인지 먼저 확인한 뒤 진행한다.
...
**가장 작고 단순한 대상**(의존성이 적고 독립적인 것) 하나를 골라 `main.md` 프로세스대로 처음부터 끝까지 수행한다. 큐 읽기·그룹 추가·문서/목록 갱신 절차는 `main.md`를 따른다.
