...
하지만 실제 서비스에 쓰이는 코드에는 학습과 무관한 것들이 많습니다. 브라우저 호환성 처리, 서버 환경 대응, 더 이상 쓰지 않는 옛 설정 지원, 드물게 일어나는 예외 처리 등이 핵심 로직을 가리고 있어서, 코드를 열어봐도 "이 라이브러리가 무슨 생각으로 이렇게 설계했는지"를 파악하기 어렵습니다.
...
구조가 파악되면, 간소화할 대상을 하나 선택하고 계획을 세웁니다. 기능을 한꺼번에 제거하지 않고, 하나씩 점진적으로 걷어냅니다.

매 단계마다 "원래 코드가 왜 이렇게 작성되었는지"를 문서로 기록한 뒤 간소화를 진행합니다. 이렇게 하면 원본의 의도를 놓치지 않으면서도 핵심만 남길 수 있습니다.
