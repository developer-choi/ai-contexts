...
2. **SKILL.md 드래프트 작성**: 단계형 절차 (맥락 파악 → 의견 수집 → 검증 → 작성 → 확인)
...
4. **말투 규칙 축적**: 사용하면서 tone-guide 백로그(`backlog/this/tone-guide.md`)에 규칙 추가
5. **커밋**: 실사용 피드백 반영 후 최종 커밋

### 관련 백로그

- `backlog/this/pr-review.md` — 이 스킬의 원본 백로그. 코멘트 작성 부분은 이 스킬이 담당. 리뷰 플로우(code-review 먼저 돌리기 등)는 별도.
- `backlog/this/tone-guide.md` — 말투 규칙이 충분히 쌓이면 별도 contexts 파일로 분리 예정
- `backlog/this/pr-comment-receive.md` — 코멘트 수신 대응은 별도 스킬(pr-comment-respond)로 만들 예정
