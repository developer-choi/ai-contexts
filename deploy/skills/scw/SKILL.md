---
description: 스킬을 만들거나 개선하고, 프롬프트 문서를 검증한다. 스킬 생성, 스킬 개선, SKILL.md 작성, 문서 검증, 리뷰, 검수, 점검, 벤치마크, eval, 테스트 요청 시 반드시 이 스킬을 사용한다.
...
- 컨텍스트 효율성 — 서브에이전트 활용 시, 컨텍스트 비용이 최적인가 (해당 시에만)
  - 메인이 이미 보유한 컨텍스트를 서브에이전트가 중복 로드하는가
  - 반복 사이클에서 이전 반복의 맥락이 유실되는가
  - 서브에이전트 위임이 필요한 이유가 명확한가 (컨텍스트 격리? 병렬화? 객관성?)
