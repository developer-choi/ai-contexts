...
위임을 쓸 때 가리키는 절 이름을 그 문서에서 grep으로 확인한 뒤 적는다. 재기술을 걷어내는 목적이 "한 곳만 보게 하기"인데, 그 한 곳을 못 찾게 만들면 걷어내기 전보다 나쁘다 — 재기술은 틀리기라도 하지만 도착 못 하는 위임은 아무것도 남기지 않는다.
...
**after**: 목적지에서 헤딩을 grep해 확인한 이름으로 적는다.
