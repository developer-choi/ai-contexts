...
계획에 따라 코드를 작성하고 커밋합니다. 커밋마다 코딩 컨벤션 리뷰를 병렬로 수행하여, 최종 점검 전에 문제를 미리 잡습니다. 구현 과정의 세부 맥락이 메인 컨텍스트를 오염시키지 않도록 서브에이전트에 위임합니다.

- [implementation](implementation/README.md) — 서브에이전트로 코드 작성을 위임하여 메인 컨텍스트를 보호
- [code-review](code-review/README.md) — 커밋마다 컨벤션 리뷰를 병렬 수행

### Step 6. 최종 점검

구현이 끝난 결과물을 여러 관점에서 검증합니다. 계획 대비 누락이 없는지 확인하고, 코딩 컨벤션별로 심층 리뷰를 수행합니다.

- [code-review](code-review/README.md) — 컨벤션과 품질 기준에 따라 코드 리뷰
