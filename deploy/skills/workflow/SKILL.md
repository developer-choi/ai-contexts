...
| **WRITING_IDEATOR** | PR_{N}_PLAN.step-3 종료 (초안 트리거) | `pr{N}/persistent/overview.md` + `pr{N}/persistent/decisions.md` (step-3 초기본, 토론 없으면 부재 가능) + `pr{N}/persistent/reference.md` | `pr{N}/consumable/pr-body.md` **초안**(배경·문제·접근·근거; 상세 코드블록 제외) — overview는 persistent라 **읽기만**, 어느 소비처도 삭제하지 않음 | 후속 spawn 없음 (REFINER는 IMPL·step-6 후 별도 트리거). per-PR·유연 타이밍 | **코드 워크트리 무관 — main repo `/plan/` 절대경로 참조** (`step-4.md:24`). 구현 맥락 없이 계획 산출물 기반 초안 | **Opus** — 계획만 보고 사용자 의도를 PR 본문 배경·근거로 녹여야 함, 의도 오독 비용 큼 |
...
### project.md 산문·초기 셋업 CLI는 사용자가

위임했을 때 검증 왕복이 직접 하는 것보다 비싼 두 가지는 그 자리에서 사용자에게 넘긴다.

- **project.md의 산문 서술** — 프로젝트 설명·설계 결정 서술처럼 이어 쓰는 글은 사용자가 쓴다. AI는 넣을 항목·방향을 후보로 제시하고 기다린다. **완성문을 지어놓고 "이대로 갈까요"로 승인받는 방식도 대상이다** — 사용자가 손보느니 처음부터 쓰는 게 빨라 그 왕복이 그대로 낭비가 된다.
- **프로젝트 골격을 만드는 초기 명령** — `git init`, gitignore 생성, 초기 커밋, 프로젝트 생성기(`yarn create vite` 류). 실행할 명령을 안내하고 사용자 실행을 기다린다.

경계:

- TODO·발견 항목·기술 결정 후보 같은 **리스트**는 AI가 project.md에 직접 기록한다. 넘기는 것은 산문뿐이다.
