...
- 각 step은 작업 종류에 따라 적절한 **하위 스킬을 선택**하는 오케스트레이터이거나, 그 자체가 실행 로직
...
### 선행조건 자가체크
- 각 step은 필요한 선행조건(컨벤션 문서, 테스트 스크립트, 리뷰 규칙 등)을 스스로 확인한다
- 필요한 파일이 없으면 해당 step에서 사용자에게 요청하고, `/plan/background/`에 저장한다
