...
PLANNING_SESSION의 모든 산출물(overview·logic·interface·implementation·test-cases·markup 등)에 적용한다. 위 「자가 검토 필수」의 1:1 대조와 별개 갈래로 보고, 두 점검을 모두 통과해야 산출물 종료다. 반복 횟수·수렴 기준은 글로벌 규칙 「검증은 수렴할 때까지 반복」을 따른다.
