---
name: session-retro
description: 세션 로그에서 대화 타임라인을 뽑고(단계 1), workflow를 타고 진행한 세션이 과제 전체 종료 후 시간소비·위임·병렬 3관점으로 자기 자신을 회고한다(단계 2). 제출용 대화내역이 필요할 때는 단계 1만 단독으로 쓴다. 호출형 — 호출 컨텍스트가 안내·호출한다.
---
...
세션 로그에서 대화 타임라인을 뽑아 두 용도로 쓴다 — 과제 전체가 끝난 뒤 각 세션이 **자기 자신**을 회고해 "어디서 시간이 샜나 / 위임이 이득이었나 / 병렬 가능했나"를 다음 작업에 반영하는 것이 주 용도이고, 회사가 세션 대화 내역 제출을 요구할 때 외부에 낼 문서를 만드는 것이 다른 하나다.
...
3. **AI 장문 요약 치환** — 산출 md에서 150자 초과 AI 블록을 서브에이전트로 `[요약]` 1~3줄 치환 (기준은 아래 「정제 규칙」). 다른 블록·헤더·구분자·순서·개수는 그대로 두고, 치환 개수를 보고받아 확인한다.
...
AI 요약(절차 3)까지 끝난 평면 타임라인에, 구간 시작시각+헤딩 배열(`SEGS`)로 각 구간 첫 블록(`**{시각} · `) 앞에 `## ` 헤딩을 삽입한다. 삽입 후 **블록 총수·`##` 개수·MISS 0을 반드시 검증**한다. (합계는 마지막 라인이 아니라 상단 메타 인용 블록 바로 아래 `> **합계**:` 한 줄로 둔다.)
...
- **AI 발화 150자 초과 → `[요약]` 1~3줄** — 핵심 주장·결정·제안·질문을 담기("설명했다" 금지). 이하는 전문 유지. 치환했으면 상단 메타 인용 블록에 그 사실을 한 줄로 밝힌다.
