...
커밋 분할(`### N` 순번이 곧 구현 순서)·방침·**행동 결정 커버리지 표**(「`it.todo` 매칭 게이트 > PLAN 시점」)·gotcha·근거.

코드·다른 절차에 이미 있는 것은 담지 않는다 — 구현 순서는 커밋 순번이, `it.todo` 매핑은 stub `*.test.tsx`가, 종료 점검은 step-5 「마무리」의 게이트가 각각 진실 원천이다.
...
- **이 표의 `it.todo` 전사는 「양식」의 전사 금지에 대한 예외다** — 상위 결정(코드에 없는 것)과의 대조표라 양쪽을 한 셀에 놓아야 빈칸이 누락으로 보인다. 코드만 옮겨 적는 나열과 성격이 다르므로 지우지 않는다.
