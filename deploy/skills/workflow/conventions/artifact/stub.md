...
**마크업 예외**: 마크업 스타일(`.module.scss`)은 MARKUP 세션이 figma 0건으로 완성하므로 **stub 대상이 아니다**(확정). 컴포넌트 `.tsx` 시각 구조(아래 컴포넌트 트리 H/B/F 예시 포함)의 step-4 stub 여부는 MARKUP 완성 마크업의 PR 반입 모델과 함께 **후속 정의**한다 — 그 정의 전까지 아래 `.tsx`·구조 가시화 예시는 잠정. 확정 stub 대상은 **로직**(hook·test·type·fixture)이다.
