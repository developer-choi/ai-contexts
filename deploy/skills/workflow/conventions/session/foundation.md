...
**컨벤션 소스 스캔 (이연분)**: `background/retained/conventions-index.md`가 없으면(레포를 세운 뒤 BG가 스캔을 건너뛴 경우), 폴더 마이그레이션 착수 전에 step-1 「컨벤션 소스 수집」의 이름 스캔·인덱스 생성을 수행한다 — 레포의 CLAUDE.md·docs 류가 폴더 마이그레이션 결정의 입력이다.
