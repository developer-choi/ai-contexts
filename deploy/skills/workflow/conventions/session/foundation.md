...
**진입 전 베이스라인 확인 필수**: 착수 전 기존 커밋 author를 확인한다. 회사 제공 베이스라인 커밋이 폴더 마이그레이션·부분 마크업·설정 일부를 이미 포함할 수 있으므로, 마이그레이션을 기계적으로 수행하지 말고 「회사 제공 베이스라인 커밋」 절을 먼저 적용한다.
...
**판별 — author가 1차 기준**: 기존 커밋의 author를 확인한다. 회사 author 커밋 = 제공 베이스라인, 후보자 본인 git user(과제 진행 계정) 커밋 = 내 작업. "진입 시점 기존 커밋 = 베이스라인" 추정은 이전 세션이 후보자 커밋을 이미 쌓은 경우 오판하므로 author 식별을 우선한다.
