...
모드 무관 공통 절차는 본 파일에 둔다. **디자인 진실 원천·재료·검사 방법은 모드마다 다르므로 모드 파일에 분리**한다 — figma 쓰는 모드(채용·실무)는 [figma.md](figma.md), figma 대신 사용자가 미리 만든 마크업 시안을 쓰는 모드(개인)는 [personal.md](personal.md). 모드가 늘면 모드 파일만 추가하고 본 파일은 건드리지 않는다.
