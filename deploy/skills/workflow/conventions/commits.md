...
| 2차 정리 (FINALIZE, 스택 전역 1회) | 슬라이스·도메인 기준 메시지 재작성 + 외부 독자 톤 최종화 |

- `<scope>`는 프로젝트 commitlint scope-enum에 따라 결정
- 커밋 소속(어느 PR인지)은 접두사가 아니라 **브랜치**(각 PR 브랜치의 base..tip 범위)로 식별한다 — 스택에서 PR=브랜치
...
큰 작업은 여러 PR로 쪼개고 **각 PR 브랜치는 앞 PR 브랜치 위에 얹는다(스택)** — main에서 각자 뻗지 않는다 (base 결정은 [../steps/step-4.md](../steps/step-4.md) 「사전 준비」). 구현 페이즈는 머지 없이 도미노로 진행하므로, 종료 페이즈(FINALIZE) 진입 시점엔 스택의 어느 PR도 아직 머지되지 않았다. 아래 메시지 최종화(스택 전역 메시지 재작성)·replace(오배치 재배치)가 성립하는 근거가 이 미머지 스택 전제다. 커밋 소속은 브랜치(base..tip)로 식별한다.

## PR 간 시간순 섞임 vs 오배치

두 현상을 구분한다 — 처리가 정반대다.

- **시간순 섞임 (소속은 맞음)**: 커밋이 원래 자기 PR에 있는데 시간 순서만 다른 PR 커밋과 뒤섞임. **재배열하지 않는다** — 스택에서 PR 소속은 브랜치(base..tip)로 식별되고 커밋 시간순은 식별 가치가 없으므로, 시간순 재배열은 순이익 없는 추가 history rewrite 부담이다.
...
### 2차 정리 대상 경계 — 조기 개별 머지분 제외

FINALIZE 2차 정리(replace + 메시지 최종화) "전 커밋 일괄" 대상은 **미머지 스택**이다. 앞 PR에 독립인 PR을 스택 완성 전에 개별 조기 머지하는 것(문서 기본 흐름 밖 1% — 사용자 수동)은 FINALIZE 일괄 조작에서 빠진다. 조기 머지분은 머지 직전 **그 PR만 수동으로 메시지 정리** + (스택 상류 섞임 방지) base retarget 또는 상류 선머지로 처리한다.
