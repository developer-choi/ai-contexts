...
목업/시안이 포함되어 있지 않은 경우("동작하는 앱을 주고 문제를 찾아 고쳐라"형 등)에도 페이지별 분석을 그냥 건너뛰지 않는다 — step-1.2 「플래그 판정 명시」의 사용자 확인 질문으로 코드기반 모드 fallback 여부를 정한다. 채택되면 위 UI 목업 분석 절차를 기존 코드 읽기로 대체해 동일하게 수행한다.
