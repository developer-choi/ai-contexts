...
**코드기반 모드**는 목업·기획서가 없고 기존 구현 코드가 있는 경우입니다(기존 동작 코드 개선형 채용과제 등) — step-1.2 「플래그 판정 명시」에서 사용자가 이 모드를 확인한 경우에 적용합니다. 자료를 받아 분석하는 대신, 대상 코드를 읽고 기획 체크리스트를 코드에 대입하여 역산합니다(코드 리딩이 자료 참조를 대체). 이후 「같이 분석 → 논의 → 확정」 절차는 동일하되, 기준이 "받은 자료"가 아니라 "코드에서 읽어낸 현재 동작"입니다.
