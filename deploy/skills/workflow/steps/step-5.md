# Step 5: 구현

> **이 단계의 목표: 구현 방침에 따라 코드를 작성한다**

메인 에이전트는 직접 코드를 읽거나 작성하지 않는다. 아래 서브에이전트들을 오케스트레이션한다.

---

## 스킬 로드

| 순서 | 조건 | 스킬 | 설명 |
|:---:|------|------|------|
| 1 | 구현 | [implementation](../implementation/SKILL.md) | 코드 작성 + 커밋 |

---

## 서브에이전트 구성

- **implementation 서브에이전트** — 코드 작성 + 커밋
- **리뷰 서브에이전트** N개 — 컨벤션을 주제 단위로 묶어 생성. 그룹핑은 AI 재량. **병렬 실행**

---

## 사이클 (phase마다 반복)

1. implementation 서브에이전트에 구현 지시 → phase 완료 후 반환
2. 사용자에게 보고
3. 리뷰 서브에이전트들에게 diff 전달 → 리뷰 결과 반환
4. 리뷰 결과를 사용자에게 보고
5. 수정 필요 시 implementation 서브에이전트에 수정 지시
6. 다음 phase

---

## [CRITICAL] 한번에 전부 구현 금지

단계별로 나눠서 진행하고, 각 단계 완료 후 사용자 리뷰를 받는다.

### 필수 승인 체크포인트

**테스트 코드 작성 후** — 테스트 코드는 이후 구현의 TDD 목표 기준이 된다. 반드시 사용자 승인을 받은 뒤 구현에 착수한다.

---

## 참고 컨벤션

[coding-standards/README.md](../../../contexts/coding-standards/README.md)를 참조하여 리뷰 서브에이전트에 전달할 컨벤션을 확인한다.

---

## 보고 내용

각 phase 완료 시:
- 커밋 목록
- 리뷰 결과 요약
- 수정 사항 (있는 경우)

## gotchas

- 기존 모듈을 대체할 때 바로 삭제하지 말고 `@deprecated` 처리 먼저. 다른 곳에서 참조 중일 수 있다.
- 새 파일/모듈을 만들기 전에 프로젝트에 같은 역할의 코드가 이미 있는지 확인한다. 기존 API, 타입, 컴포넌트를 재사용할 수 있으면 새로 만들지 않는다.
