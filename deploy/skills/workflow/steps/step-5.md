...
- 모든 슬라이스 사이클 종료 후에도 stub 커밋부터 IMPL/리뷰 수정 커밋이 그대로 보존된 상태로 step-6에 진입한다. 커밋 정리·재정렬은 step-6 「Step 6.4. 커밋 정리·재정렬」에서 **AI 리뷰 + 사용자 리뷰 완료 후** 수행한다 (사용자가 stub→IMPL 변환 diff를 리뷰할 수 있어야 하므로)
...
- 전체 리뷰 완료 후, stub 커밋부터 IMPL/리뷰 수정 커밋이 그대로 보존된 상태로 step-6에 진입한다. **커밋 정리·재정렬은 step-5에서 수행하지 않는다** — step-6 「Step 6.4. 커밋 정리·재정렬」에서 AI 리뷰 + 사용자 리뷰 완료 후 일괄 수행
- Lead가 사용자에게 결과 보고
