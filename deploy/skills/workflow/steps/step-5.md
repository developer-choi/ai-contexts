...
step-5는 아래 역할로 팀을 구성한다. **진입 시 고정 spawn이 아니다** — 구현자는 이 PR이 만드는 산출물(마크업/로직)에 해당하는 것만 띄우고, 리뷰어 구성은 Step 5.3에서 [impl-review-loop](../impl-review-loop/SKILL.md)를 호출할 때 그 A 메커니즘이 결정한다 (오라클형이면 축 A Reviewer 미spawn, 대조형이면 Reviewer ↔ 구현자 루프).
...
구현·리뷰는 [impl-review-loop](../impl-review-loop/SKILL.md) 엔진을 호출해 0건까지 수렴시킨다. Lead는 구현자별로 아래 인자를 주입한다 (구현자·재료·팀 컨텍스트는 Step 5.1·5.1.1 참조). 두 축의 순서·병렬은 엔진이 A 메커니즘의 성질로 정하므로 호출자가 지시하지 않는다.

| 구현자 | 진실검사 A (메커니즘) | 규칙검사 B | 증분 단위 |
|---|---|---|---|
| Markup Implementer | figma 원본 직접 fetch — Figma Reviewer ↔ 구현자 대조 루프. 대조형(실행 오라클 없음). 검증 기준은 [markup-spec.md](../conventions/artifact/markup-spec.md) 「검증 기준 — figma 원본 직접 fetch」 | Coding-Standards ×N + Advanced (마크업 rules, Step 5.1.4 분할) | 마크업 커밋 |
| Feature Implementer | 테스트 실행 green + `it.todo` 커버리지. 오라클형(실행이 곧 판정). 종료 커버리지는 [implementation-spec.md](../conventions/artifact/implementation-spec.md) 「`it.todo` 매칭 게이트」 | Coding-Standards ×N + Advanced (로직 rules, Step 5.1.4 분할) | 로직 커밋 |
