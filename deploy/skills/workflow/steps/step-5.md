# Step 5: 구현

> **이 단계의 목표: 구현 방침에 따라 코드를 작성한다**

`/plan/pr{N}/overview.md` 구현 방침을 바탕으로 **코드 작성 및 커밋** 단계입니다.

**목표**:
- overview.md에 정의된 기능 구현
- 작은 커밋 단위로 리뷰하기 쉽게 만들기

---

## 커밋 순서 가이드

다음 순서로 커밋을 쌓는 것을 권장합니다. 더 좋은 순서가 있다면 사용자에게 제안하세요.

### Step 5.1. 사전 작업 (Conflict 방지)
**1. Deprecated 처리**
- 기존 모듈 대체 시 바로 삭제 말고 `@deprecated` 먼저
```typescript
/**
 * @deprecated 대신 NewModule 사용
 */
function deprecatedSomeFunction() {}
```

2. **기존 코드 리팩토링**: 파라미터 타입 변경 등 (나중에 Conflict 방지)
3. **파일 분리**: 관심사/책임 많아지면 역할 기반으로 분리

### Step 5.2. 기초 공사
4. **타입 정의**: 모든 구현의 기준
5. **API 함수**: 백엔드 API 준비됐으면 호출 함수 작성
6. **단위 모듈/컴포넌트 마크업**:
   - 공통 UI 컴포넌트 (도메인 비종속)
   - 도메인 기초 UI 컴포넌트 (도메인 종속)
   - 유틸리티/포매팅 함수
   - 상수
   - 상태별 UI (Empty, Loading, Error)
   - **우선순위**: 공통 모듈 → 도메인 종속 모듈

### Step 5.3. 통합
7. **Mock 기반 통합**: Mock API, Hook 구현, 페이지 조립

### Step 5.4. 연동
8. **실제 API 연동**: Mock → 실제 API, 에러 처리, 엣지 케이스

---

## [CRITICAL] 구현 진행 방식

**⚠️ 한번에 Step 5.1~5.4 전부 구현 금지!**

### Step 5.5. 참고 컨벤션 탐색 (최초 1회)

[coding-standards/README.md](../../../contexts/coding-standards/README.md)를 참조하여 이번 PR 작업에 필요한 컨벤션 파일을 찾고, `/plan/pr{N}/overview.md`에 리스트업합니다.

```markdown
## 참고 컨벤션
- `폴더경로/파일명.md`
- `폴더경로/파일명.md`
```

### Step 5.6. 참고 컨벤션 읽기 (최초 1회)

위에서 리스트업한 컨벤션 파일의 **내용을 읽습니다**. 컨벤션 내용이 구현 방식이나 커밋 단위에 영향을 줄 수 있으므로, 반드시 Step 5.1~5.4별 계획 수립 전에 완료합니다.

### Step 5.7. Step 5.1~5.4별 반복

1. **각 Step(5.1~5.4) 시작 전 plan mode로 진입**하여 다음을 포함한 계획 제출:
   - 수정/생성할 파일 탐색
   - **위 커밋 순서 가이드를 기반으로 커밋 단위 분리**: 각 커밋에 포함될 파일과 변경 내용을 명시
   - 예시 형식 (순서는 위 커밋 순서 가이드 참고):
     ```
     Commit 1: 커밋 메시지
     Commit 2: 커밋 메시지
     Commit 3: 커밋 메시지
     ```
2. 사용자 승인 후, **계획된 커밋 순서대로 1커밋씩 실행**
   - 1커밋 = 코드 작성 → 빌드 확인 → prettier → 즉시 커밋
   - **prettier**: 프로젝트 최상위에 prettier 설정 파일이 있으면, staged 파일에 `npx prettier --write $(git diff --cached --name-only)` 실행 후 재스테이징
   - 다음 커밋 작업은 이전 커밋 완료 후 시작
   - "커밋할까요?" 질문 금지. 바로 커밋할 것
3. 커밋 누락 확인 (uncommitted 변경사항 없는지 점검)
4. 모든 커밋 완료 후 사용자 리뷰
5. 다음 Step(5.1~5.4) 반복

**이유**: 한번에 구현하면 중간 문제 발견 시 많은 코드 수정 필요. Step 5.1~5.4별 리뷰로 빠른 발견/수정

### Step 5.1~5.4 각 완료 전 자가 점검

사용자에게 보고하기 전, 다음을 수행:
1. overview.md의 **참고 컨벤션** 파일을 다시 열어 작성한 코드와 대조
2. 위반 사항이 있으면 수정 후 커밋
3. 응집도/결합도 관점에서 리뷰

### 보고 내용

각 Step 5.1~5.4 완료 시 사용자에게 다음을 요약하여 보고:

- 이번 sub-step 커밋 목록
- 자가 점검 결과 (컨벤션 위반 수정 여부, 응집도/결합도 판단)

