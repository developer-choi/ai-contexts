...
> **Plan mode 필수**. AI가 제시한 과제는 사용자 승인을 거쳐야 하며, 승인된 과제만 [구현 단계](step-4.md)에서 구현한다 (실행 또는 stub 분해).
...
`/plan/pr{N}/` 하위와 `/plan/background/`를 탐색하여 기존 산출물을 읽고, overview.md를 작성한다. 이 단계에서는 읽기만 하며, 원본 산출물을 삭제하지 않는다 (소비는 [구현 단계](step-4.md)에서).

이 단계는 "무엇을 구현할지"를 결정한다. "어떻게 구현할지"는 [구현 단계](step-4.md)에서 다룬다.
