...
- **산출물**: 토론을 수행한 경우 `/plan/pr{N}/decisions.md` — [/discussion의 토론 결과 양식](../discussion/SKILL.md#토론-결과-정리)을 따른다
...
보고 전에 산출물 파일이 실제로 `/plan/pr{N}/`에 생성되었는지 확인한다 (`overview.md` 필수, `decisions.md`는 토론을 수행한 경우에만). 구두 보고만으로 완료 처리하지 않는다.
