...
> **Plan mode 필수**. AI가 제시한 과제는 사용자 승인을 거쳐야 하며, 승인된 과제만 Step 4에서 구현한다 (실행 또는 stub 분해).

---

### 스킬 로드

아래 조건에 해당하는 스킬을 **모두** 로드한다. 여러 개면 위에서부터 순서대로 하나씩 실행한다.

| 조건 | 스킬 | 실행 | 설명 |
|------|------|------|------|
| 채용과제 | [prefer-packages](../requirement-review/recruitment/prefer-packages.md) | — | 선호 패키지 참조 |
...
상단에 큐 모델 자가 정리 안내문을 박는다 ([conventions/plan-folder.md](../conventions/plan-folder.md) 「consumable/ 산출물 자가 정리 안내문」). 본 파일 모든 절의 **유일·마지막 소비자는 WRITING_REFINER** (PR 본문 확정) — WRITING_IDEATOR는 초안 시점에 읽기만 하고, WRITING_REFINER가 PR 본문으로 소비·삭제한다 (consumable 큐 모델).
...
| `markup.md` | **Figma 원본 링크 인덱스(사용자 입력)** + 토큰 매핑표·매칭표 (개인 모드는 figma 없어 N/A — [markup-spec.md](../conventions/artifact/markup-spec.md)) | 코드 블록 없음 (링크·도표만) |
| `implementation.md` | 구현 순서·커밋 분할·회귀 체크리스트 | 신설 시그니처·함수명 OK (rename류 기존 식별자 전사 X — grep 일괄치환 지시로) |
...
- WRITING_IDEATOR 초안 반영 후 WRITING_REFINER가 PR 본문 "Known issues / Follow-up" 절로 소비 (overview.md는 consumable 큐 모델)
...
`/plan/background/consumable/project.md`가 존재하면 **현재 PR 절을 `/plan/pr{N}/consumable/overview.md`로 소비한다** (TODO 포함 이관) 후 사용자에게 안내. project.md는 순수 큐라 소비 후 처리는 [conventions/plan-folder.md](../conventions/plan-folder.md) 「소비→삭제 메커니즘 SSOT」를 따른다 — 제목·pointer를 남기지 않는다.
