...
이전 PR의 코드·결정을 컨텍스트로 참조하면, 자기가 이미 로드한 컨벤션 안에서 비판적으로 검토한다.
...
| `implementation.md` | `pr{N}/persistent/` | 구현 순서·방침·`@deprecated` 흐름·it.todo 매핑·회귀 체크리스트 | 대부분 작성됨 |

**implementation.md 회귀 체크리스트**: it.todo ↔ it() 매칭, 테스트 명령 통과, PR 성격별 추가 항목. (위치·소비·폐기는 SKILL.md 「/plan/ 폴더 구조」 implementation.md 항목 참조.)
...
- 파일 경로·폴더 구조·파일명 컨벤션 (예: `{domain}-api.ts` 네이밍, `entities/{domain}/api` 폴더 구조)
...
IMPL 단계 라이프사이클과의 관계: 본 룰(재추가 금지)은 step-5 「TODO 주석 해결 시 즉시 삭제」 룰과 동일 정신 — 사용자가 TODO 주석을 정리한 시점이 곧 "그 결정의 의도 확정"이므로 그 후에는 TODO 주석 재생성을 하지 않는다.
...
- ✓ 영속 파일: `docs/conventions/*`, `docs/ARCHITECTURE.md`, `_fsd/.../*`, `plan/{prN}/persistent/decisions.md`, `plan/{prN}/persistent/reference.md`, `plan/{prN}/persistent/implementation.md`
- ❌ 영속 아님(stub 주석 인용 부적합): `plan/{prN}/consumable/overview.md` (소비 시 폐기), `plan/{prN}/retained/markup.md` (step-6.5에서 폐기)
...
| 디자인 검수 후 확정 — 별도 일정 (라운드 본 PR 외) | backlog md 「열려있는 질문」 | 본 PR 외부 |
| 백엔드 합의 후 nullable 재조정 — IMPL 안에서 끝 | `TODO [AI_IMPL]` | 본 PR 안 |
| 백엔드 합의 지연 가능 — 본 PR 종료 후 잔존 | backlog md 「열려있는 질문」 | 외부 의존성 |
...
목적: step-5의 Implementer와 Reviewer가 본 링크로 **figma 원본을 직접 fetch**해 검증 (SKILL.md 「검증 기준 = 진실 원천」).
