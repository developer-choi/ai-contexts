...
| `markup.md` | 토큰 선택 근거, 디자인 의도, 매칭표, **「IMPL 시점 Figma 확인 체크리스트」 절** | UI 컴포넌트가 있는 PR이면 필수 (.module.scss stub은 IMPL이 본 체크리스트 보고 토큰 매핑을 채움). 그 외 PR은 narrative 한 문장 이상 있을 때만 |
...
- 조건은 placeholder 변수 (`const isLoading = false; // TODO [AI_IMPL]: logic — plan/{prN}/logic.md "절명"`)
