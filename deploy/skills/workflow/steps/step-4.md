...
이후 이 단계의 모든 작업(stub 파일 생성·stub 커밋 포함)은 새로 만든 워크트리 안에서 수행한다. 이전 step 산출물(`/plan/pr{N}/consumable/overview.md`, `/plan/pr{N}/persistent/` 하위 등)이 base 브랜치에 commit되어 있지 않아 워크트리에 보이지 않으면, 작업 시작 전에 워크트리로 가져온다. 메인 세션이 직접 cwd를 옮길 수 없으면 사용자에게 워크트리 디렉토리에서 새 세션을 띄워 이어가도록 안내한다.
...
채용과제의 페이지 이미지(`/plan/pr{N}/retained/` 하위 `page*.png`)는 해당 PR의 **stub `.tsx` 작성 시 + `markup.md` 작성 시** 시각적 참조로 사용한다. 소비 대상이 아니며 삭제하지 않는다.
...
5. 선별된 컨벤션·패턴 경로를 `/plan/pr{N}/persistent/reference.md`에 통합 명시한다. 각 stub 파일·잔존 md에는 적지 않는다.

결정 가능한 컨벤션은 전부 stub 코드와 `reference.md`에 녹인다 (파일 경로·네이밍은 stub 자체로, 토큰 사용 규칙·에러처리 전략 같은 명시 규칙은 reference.md로). 이를 통해 이후 구현 단계에서 컨벤션 원본을 가급적 덜 볼 수 있게 한다.

### [CRITICAL] 컨벤션 1차 소스 직접 grep 의무

stub 폴더 구조·파일 배치·네이밍·import 경로를 결정할 때 관련 컨벤션 1차 소스를 **직접 grep**한 후 결과를 stub 주석 `[Convention]` 블록에 인용한다. "안다고 가정"·"이전 세션 기억"에 의존하지 않는다.
...
| `markup.md` | `pr{N}/retained/` | **「Figma 원본 링크 인덱스」 절(사용자 입력)** + 토큰 매핑표, 매칭표, **「IMPL 시점 Figma 확인 체크리스트」 절** | UI 컴포넌트가 있는 PR이면 필수. 사용자가 figma 컴포넌트·상태별 URL을 직접 입력. step-5 Implementer·Reviewer가 본 링크로 figma 원본을 직접 fetch (검증 기준 = figma 원본, AI 산출물 아님). 그 외 PR은 생성 안 함 |
| `implementation.md` | `pr{N}/retained/` | 구현 순서·방침·`@deprecated` 흐름·it.todo 매핑 | 대부분 작성됨 |
...
### stub 범위 — 모든 파일·함수·컴포넌트 stub 필수

stub 단계의 두 원칙:

1. **PR에서 만들 모든 파일·함수·컴포넌트는 stub 필수** — 외부 공개 여부 무관. 내부 헬퍼·내부 컴포넌트도 시그니처 + `throw new Error('not implemented')`로 stub. PR 안 구조(컴포넌트 트리·함수 호출 그래프)가 한눈에 드러나야 IMPL이 본문만 채우면 됨.
2. **코드로 표현 가능한 결정은 모두 stub에 박는다** — 결정됐고 코드 표현이 가능하면 별도 narrative 파일이 아니라 stub 코드에. 단, **코드 분량이 크거나 짧은 한글 명세("이 함수는 X 케이스에 Y 반환")가 더 명확한 경우** stub 본문을 다 적지 않고 `// TODO [AI_IMPL]:` 주석에 한글 요약으로 적는다. 예: `// TODO [AI_IMPL]: 페이지네이션 — cursor null이면 첫 페이지, 이후 prev cursor 사용`
...
이유 (2): 코드가 narrative보다 진실 원천에 가깝다. 결정됐는데 narrative로 적으면 stub 코드 + narrative 양쪽 동기화 부담 (scw 「코드와 1:1 중복 작성 금지」 정신).

step-4 vs step-5 경계의 항목별 매핑·점검 항목은 scw/specialized/workflow.md 「step-4 vs step-5 코드 작성 경계」 참조.
...
예시 (2) — 코드 표현 가능한 결정은 stub:

- ❌ Bad — 결정된 사항을 별도 narrative 파일에 적음
- ✓ Good — 결정된 사항은 stub 코드에 박음
  - `useUserQuery.ts` stub: `useQuery({ ..., staleTime: 5 * 60 * 1000 })`

예시 (2-b) — 분량 크거나 한글 명세가 더 명확하면 stub 본문 대신 한글 요약 주석:
...
외부 의존성 가정도 코드 표현 가능 — TypeScript type stub:
...
| Context Provider | provider 트리 위치·value 타입 결정됐으면 stub 작성. 미결이면 사용자 질문 → 결정 후 stub. 결정 못 하면 placeholder + `// TODO [AI_IMPL]:` 한글 요약으로 step-5에 미룸 |
...
#### [CRITICAL] 적용 조건 해석 — 명시된 변수만으로 판단

상기 「제외 대상」(개인 레포·채용과제)만으로 적용 여부를 판단한다. **그 외 변수를 자체 도입해 룰을 완화하지 않는다.**

자체 도입 금지 변수 예시:

- 회사 프로젝트지만 "컨벤션 문서가 별도로 없음 (`docs/conventions/*` 같은 폴더 부재) → 출처 명시 가치 낮음"으로 판단 — **금지**. ESLint·prettier·기존 코드 패턴이 컨벤션 진실 원천이며 `[Reference]`로 인용 가능
- "PR 크기가 작아 노이즈 비용이 큼 → 룰 비용 vs 효익 비대칭" 자체 판단 — **금지**. 비용 판단은 사용자 권한
- "본 stub은 컨벤션 위반 risk가 낮음" 자체 판단 — **금지**. 위반 risk 판단도 사용자 권한

적용 여부가 명시된 변수로 결정되지 않으면 사용자에게 확인한다. 자체 해석으로 룰을 가볍게 적용하지 않는다.

**회고 사례 (langdy-student PR2 stub)**: 회사 프로젝트인데도 "컨벤션 문서 부재 → 노이즈 비용 큼"으로 자체 판단해 `// 일반 주석`으로 stub 상단 작성. 사용자가 검토 단계에서 룰 위반 지적 → 별도 fixup commit으로 `/* TODO [USER_REVIEW] [Convention] [Reference] */` 블록 양식 적용. 룰 정신 해석으로 임의 완화하는 패턴은 일관성 어긋남으로 이어진다.
...
PR 이연 후속 작업(예: "PR4에서 X 추가")은 코드 안 마커가 아닌 `/plan/background/consumable/project.md`의 해당 PR(PR4) 섹션에 적는다. 본 PR 외부 의존성(백엔드 합의·디자인 검수·인프라 등 PR이 아닌 외부 작업)은 `plan/{prN}/consumable/overview.md` 「열려있는 질문」 절에서 관리. 코드 안 TODO는 본 PR 안에서 모두 해소되어야 한다.
...
**PR 이연 후속 작업은 코드 안 마커가 아닌 `/plan/background/consumable/project.md`의 해당 PR 섹션에 적는다**:

- ❌ 코드 안 `// TODO: PR4에서 만료 안내 배너 추가` 금지
- ✓ `/plan/background/consumable/project.md`의 PR4 섹션에 "만료 안내 배너" 항목 추가

본 PR 외부 의존성(PR 아닌 외부 작업)은 `plan/{prN}/consumable/overview.md` 「열려있는 질문」 절에서 관리. 코드 안 TODO는 본 PR 안에서 모두 해소되어야 한다.

**출처 인용은 영속 파일만**:

`TODO [USER_REVIEW]` / `TODO [AI_IMPL]` 모두 출처 인용 시 영속 파일만 허용한다.

- ✓ 영속 파일: `docs/conventions/*`, `docs/ARCHITECTURE.md`, `_fsd/.../*`, `plan/{prN}/persistent/decisions.md`, `plan/{prN}/persistent/reference.md`
- ❌ 영속 아님(stub 주석 인용 부적합): `plan/{prN}/consumable/overview.md` (소비 시 폐기), `plan/{prN}/retained/implementation.md`·`plan/{prN}/retained/markup.md` (step-6.5에서 폐기)
- ❌ 분배 후 삭제 예정인 1차 입력 (예: 사용자가 작성한 `page.md`, BG 세션 AI 산출물 등 step-4 「1. 잔여 산출물 소비」에서 삭제 대상). 라인 번호 인용 금지

1차 입력 인용이 필요하면 분배된 영속 파일로 우회 인용. 예: `page.md:30` → stub 코드 안 한글 요약 주석 또는 `plan/{prN}/persistent/decisions.md`의 결정 항목.
...
| PR 이연 작업 (예: PR4에서 X 추가) | `/plan/background/consumable/project.md` 해당 PR 섹션 | 다른 PR로 이관 (코드 안 금지) |

**판단 기준**: 본 PR IMPL이 처리 → 코드 안 TODO 마커. 본 PR 외부 의존성(PR 아닌 외부 작업) → overview.md 「열려있는 질문」. 다른 PR로 이연 → project.md 해당 PR 섹션.
...
- **layout 구조 속성만 PLAN에서 적는다** — 컴포넌트 의도를 표현하는 속성
- **디자인 값 속성은 PLAN에서 적지 않는다** — 토큰 변수로도, raw 값으로도 작성 금지. IMPL이 markup.md 체크리스트 보고 채움

| 분류 | 예시 속성 | PLAN | 근거 |
|---|---|---|---|
| **layout 구조** (PLAN에서 적음) | `display`, `flex`, `flex-direction`, `flex-shrink`, `flex-grow`, `grid`, `grid-template-columns/rows`, `position`, `align-items`, `justify-content`, `align-self`, `justify-self`, `min-width`, `max-width`, `width: auto`, `overflow`, `white-space`, `text-align`, `box-sizing` 등 | ✓ | 컴포넌트가 어떻게 배치되는지의 의도 — PLAN 결정 |
| **디자인 값** (PLAN에서 적지 않음) | `color`, `background`, `background-color`, `padding`, `margin`, `gap`, `border`, `border-radius`, `box-shadow`, `font-size`, `font-weight`, `line-height`, 구체적 `width`/`height` (px·rem) 등 | ❌ | Figma 디자인 시스템 토큰 매핑이 필요한 값 — IMPL 시점 결정 |

**raw 값도 토큰 변수도 PLAN 금지** — 미정인 디자인 값을 PLAN에서 `gap: 16px` 같은 raw 값으로 박지 않는다. `gap: var(--semantic-gap-lg)` 같은 토큰 변수도 PLAN에서 추측해서 박지 않는다. 모두 IMPL이 Figma 보고 채움.
...
// ❌ Bad (PLAN stub) — raw 값 박기
.row { display: flex; gap: 16px; }

// ❌ Bad (PLAN stub) — 추측한 토큰 박기
.row { display: flex; gap: var(--semantic-gap-lg); }
```

사유: 토큰명·raw 값은 Figma 디자인 검수 라운드에서 확정. PLAN 시점에 추측하면 IMPL이 다시 확인·교체해야 함 (이중 작업). PLAN은 layout 구조만 결정, 디자인 값은 IMPL이 markup.md 체크리스트로 일괄 매핑.

#### markup.md 「Figma 원본 링크 인덱스」 절 양식

UI 컴포넌트가 있는 PR이면 markup.md 상단에 본 절을 필수 작성. **컴포넌트 종류별 × 상태별로 figma 원본 URL을 사용자가 직접 입력**한다. AI가 추론으로 URL을 짐작해 박지 않는다 — 누락·오류 위험.

목적: step-5의 Implementer와 Reviewer가 본 링크로 **figma 원본을 직접 fetch**해 검증. AI 산출물(토큰 매핑표·매칭표)을 검증 기준으로 쓰는 자기증명 루프를 회피한다.
...
#### markup.md 「IMPL 시점 Figma 확인 체크리스트」 절 양식

UI 컴포넌트가 있는 PR이면 markup.md에 본 절을 필수 작성. 각 .module.scss 파일별 sub-heading + 체크박스로 IMPL이 채울 항목 명시.
...
- **step-4 자체의 stub 작성 룰(위 「4. stub 파일 작성 룰」 섹션) 준수 여부도 직접 점검** — 특히 lint가 잡지 못하는 항목:
  ...
  - **상단 출처 주석 양식** (회사 프로젝트 신규 stub 파일): `/* TODO [USER_REVIEW] [Convention] [Reference] */` 블록으로 시작하는지 (`//` 일반 주석으로 작성된 경우 룰 위반). `grep -l "^/\* TODO \[USER_REVIEW\]" {신규 stub 파일}` 또는 파일 첫 줄 확인
  - **본문 주석 마커 분리**: stub 코드 안 설명 주석이 `// TODO [USER_REVIEW]:` 또는 `// TODO [AI_IMPL]:` 마커로 분리되어 있는지 (그냥 `//` 시작하는 설명 주석이면 룰 위반)
