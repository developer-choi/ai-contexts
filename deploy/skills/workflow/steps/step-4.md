...
- **`.gitignore` 대상(예: `plan/`이 글로벌 ignore에 포함)** — 복사·심볼릭 만들지 말고 **main repo 절대경로로 그대로 참조**한다 (예: `C:\...\repo\plan\pr4\persistent\decisions.md`). 복사 시 양쪽 동기화 부담·잔존 위험. 다음 세션도 같은 절대경로로 plan을 참조하도록 진입 안내문에 경로를 명시한다
