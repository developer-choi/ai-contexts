...
- 상단 출처 블록은 `/* [stub]`로 시작한다. stub 단계 임시 마커로, IMPL 시점에 다른 `[stub]` 주석들과 함께 일괄 식별·제거할 수 있게 하기 위함
- step-5에서 stub을 실제 코드로 채우며 `[Convention]`/`[Reference]` 출처 확인이 끝났으면 블록 전체를 삭제한다 (IMPL 완료 시 stub 마커는 모두 제거)
...
상단 `/* [stub] [Convention] [Reference] */` 블록에 적는 항목:

- 파일 경로·폴더 구조·파일명에 관한 컨벤션 (예: `{domain}-api.ts` 네이밍, `entities/{domain}/api` 폴더 구조)
- 파일 전체 평행 사례 (`[Reference]`)

코드 라인 바로 위 `// [stub]:` 주석으로 분리할 항목:

- 함수 시그니처·호출 패턴·분기 패턴 등 특정 코드 라인과 1:1 매핑되는 컨벤션
...
- 변환 헬퍼 함수 위에 적용되는 컨벤션
  - 예: `pages 경계에서 SafeAsyncErrorType → 도메인 타입 정규화 — docs/conventions/04-patterns.md:131`은 `toXxxError` 함수 선언 줄 바로 위
...
// [stub]: 특정 코드 패턴 관련 컨벤션 — docs/conventions/...:line
function someHelperFollowingConvention() { ... }

export function MainComponent() {
  // [stub]: 호출 패턴 관련 컨벤션 — docs/conventions/...:line
  const result = await someConventionFollowingCall();

  // TODO: PR3에서 X 추가 (다음 PR 이연)
  return ...;
}
```
...
stub 주석(`/* [stub] ... */` 상단 블록, `// [stub]:` 인라인) 적용은 **stub 초기 생성 시점(파일 신규 작성)에 한정**한다. 기존 stub 파일의 코드 본문을 갱신할 때 stub 주석을 자동 재추가하지 않는다.

세부 조항:

- 신규 stub 파일 작성 시: 「상단 출처 주석」 + 「본문 주석은 `[stub]` / `TODO` 마커 분리 필수」 룰 적용
- 기존 stub 파일 코드 본문 갱신 시: **기존 stub 주석 상태를 그대로 보존**. 제거된 stub 주석을 다시 박지 않는다. 사용자가 직접 정리한 stub 주석은 의도된 상태로 본다
- `system-reminder`로 "modified, intentional, don't revert"가 안내된 파일은 stub 주석 재추가 절대 금지

도구 선택:

- 기존 stub 파일 본문 갱신은 **Edit 우선** (부분 수정 → 기존 stub 주석 자동 보존)
- Write는 stub 신규 생성 또는 본문 구조 자체를 전면 재작성하는 경우에만. Write로 갱신할 때는 기존 파일의 stub 주석 보존 여부를 의식적으로 확인한다

IMPL 단계 라이프사이클과의 관계: step-5의 「stub 주석 해결 시 즉시 삭제」 룰은 IMPL에서 코드 구체화하며 stub 주석을 제거하라는 룰이다. 본 룰(재추가 금지)은 PLAN 단계 안에서도 동일 정신 — 사용자가 stub 주석을 정리한 시점이 곧 "그 stub의 의도 확정"이므로 그 후에는 stub 주석 재생성을 하지 않는다.

양식 예시:

```
// ❌ Bad — 기존 파일 갱신 시 사용자가 정리한 stub 주석을 다시 박는 패턴
// (Write로 전면 재작성하며 룰만 따라 stub 주석 자동 추가)

// ✅ Good — 기존 stub 주석 상태를 그대로 보존하고, 제거된 stub 주석은 재생성하지 않음
// (Edit로 부분 수정, 사용자 정리 의도 존중)
```

### 본문 주석은 `[stub]` / `TODO` 마커 분리 필수

stub 단계에서 본문에 추가하는 설명 주석은 용도에 따라 두 마커로 분리한다.

- `// [stub]:` — stub 단계 임시 메모. IMPL 시 코드로 구체화하며 즉시 삭제 (컨벤션 출처, placeholder 의도, 구현 인지 메모 등)
- `// TODO:` — 실제 후속 작업. 다음 PR 또는 별도 이슈로 이연되며 IMPL 종료 후에도 잔존 가능

사유: IMPL 단계에서 마커로 두 종류를 즉시 구분 가능. `[stub]`은 grep으로 잔존 0건 확인하고, `TODO`는 종료 보고에 이연 사유와 함께 명시한다.

`// [stub]:` 대상:

- 함수/변수 위의 동작 설명 주석 (예: `// [stub]: 빈 값 처리 — nuqs가 자동으로 null 처리`)
- 컨벤션 출처 인라인 메모 (위 「상단 블록 vs 인라인 분리 기준」 참조)
- 함수 내부 동작 설명 주석 (예: `// [stub]: hasActiveFilter 통과 시점에 lessonId는 number 보장`)

`// TODO:` 대상:

- 다음 PR로 이연되는 후속 작업 (예: `// TODO: PR3에서 카드 그리드 추가`)
- 별도 이슈로 분리하기로 합의된 이연 항목

제외:

- 파일 최상단의 `/* [stub] [Convention] [Reference] */` 블록 주석 — 본 「본문 주석」 룰의 대상 아님. 별도 룰(위 「상단 출처 주석」)을 따라 `/* [stub]`로 시작하고 IMPL 완료 시 블록 전체를 삭제
- 코드 카테고리 마커 (예: `// 404`, `// 400`) — 기존 코드 컨벤션
- 컨벤션상 명시된 JSDoc (예: 02-layers.md의 `@param`/`@returns` 등)

원칙적으로 stub 단계에서 본문 주석을 최소화한다. 글로벌 룰 "Default to writing no comments"가 우선. placeholder 의도가 의도적으로 비명확한 경우만 `[stub]`/`TODO` 마커로 남긴다.

기존 stub 파일을 갱신할 때 사용자가 정리한 `// [stub]:` 주석을 자동 재추가하지 않는다. 본 룰의 상세는 위 「상단 출처 주석 > 재추가 금지 — 기존 stub 파일 갱신 시」 참조.
