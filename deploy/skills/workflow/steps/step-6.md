...
step-7에서 PR 본문 Test plan으로 소비한다 (consumable 큐 모델).
...
- **PR_{N}_WRITING** 후속 spawn 안내.
- **다음 PR 진입 가능 안내**(PR_{N+1}_PLAN — PR 도미노) — 현 PR이 마지막인지 먼저 `/plan/background/consumable/project.md`(PR 분할·이연 인덱스)를 읽어 판단한다. `pr{N+1}` 디렉토리 존재 여부는 보조 신호로만 본다 — 폴더가 없어도 `project.md`에 PR{N+1}+ 분할이 정의돼 있으면 마지막이 아니다. 마지막이 아니면 다음 PR의 step-3 진입 가능을 사용자에게 안내한다. (stub을 만든 경우의 PR_{N+1}_PLAN은 이미 step-4 stub 시점에 도미노로 시작됐으므로 여기서 중복 안내하지 않는다.)
- **FINALIZE 진입 안내**(마지막 IMPL일 때만) — 위 판정에서 현 PR이 스택의 마지막 IMPL이면, WRITING 안내에 더해 FINALIZE 진입도 안내한다 ([conventions/session/finalize.md](../conventions/session/finalize.md), SKILL.md 「fan-in 후속」). 마지막이 아니면 FINALIZE 안내는 출력하지 않는다.
