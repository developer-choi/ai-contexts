...
산출물: `/plan/pr{N}/user-test-cases.md`. **수동 동작 테스트 전용** — stub의 `it.todo`(자동화 단위 테스트)와는 별개이며 함수 단위 케이스는 적지 않는다.
...
#### Step 6.5.2-A. stub이 빈 껍데기인 정상 케이스
...
비대화형 환경(claude code 등 git rebase -i 불가)에서는 아래 6.5.2-B 방식을 그대로 적용해도 결과 동일.

#### Step 6.5.2-B. stub이 본문을 안고 있는 케이스
...
구현·리뷰 과정에서 새로 발생하거나 step-3 작성 시점과 달라진 의사결정을 `decisions.md`에 반영한다.
