...
## Step 6.1.6. 미배정 blanket eslint-disable 고아 점검 (백스톱)
...
- 미배정 blanket eslint-disable 고아 목록 (6.1.6 — 고아가 있는 경우만)
...
step-6 완료 = **PR_{N}_IMPL 세션 종료**. 이 시점에 아래 후속을 안내한다 (step-5에서 조기 출력하지 않고 여기서 한다 — IMPL이 완전히 끝나야 시그니처가 확정되기 때문). SKILL.md 「세션 spawn 안내 메커니즘」 절차를 발동한다.

- **WRITING_REFINER** 후속 spawn 안내.
- **본 PR에 의존하는 PR의 진입 가능 안내** — `/plan/background/consumable/project.md`를 읽어 **의존 항목에 본 PR이 적힌 PR**을 찾는다(직전 번호가 아닐 수 있고 여럿일 수 있다). 그런 PR이 있으면 각각의 step-3 진입 가능을 사용자에게 안내한다. 본 PR이 stub을 만들었다면 그 PR들은 이미 step-4 stub 시점에 출발했으므로 여기서 중복 안내하지 않는다. **세션을 새로 띄우라는 안내가 아니라 출발 게이트가 풀렸다는 안내다** — PLAN spawn 자체는 BG가 PR 확정 시점에 이미 안내했다.
- **마지막 IMPL 판정** — 같은 `project.md`를 기준으로 아직 IMPL이 안 끝난 PR이 남았는지 본다. `pr{N}` 디렉토리 존재 여부는 보조 신호로만 본다 — 폴더가 없어도 `project.md`에 정의된 PR이 남아 있으면 마지막이 아니다.
- **FINALIZE 진입 안내**(마지막 IMPL일 때만) — 위 판정에서 현 PR이 마지막 IMPL이면, WRITING_REFINER 안내에 더해 FINALIZE 진입도 안내한다 ([conventions/session/finalize.md](../conventions/session/finalize.md), SKILL.md 「fan-in 후속」). 마지막이 아니면 FINALIZE 안내는 출력하지 않는다.
