...
- **커밋 로그 분석**: `git log`로 추가된 커밋 식별. 메인은 `git log`, `git diff --stat`(파일 목록)까지만 사용 — 실제 코드 diff는 서브에이전트가 리뷰
...
컨벤션 문서 1개당 서브에이전트 1개를 실행한다. **병렬 실행**. 각 에이전트는 전체 수정 파일을 해당 컨벤션 관점에서만 리뷰한다 (좁고 깊게). Step 5의 커밋별 리뷰(넓고 얕게)와 상호보완적이다.
