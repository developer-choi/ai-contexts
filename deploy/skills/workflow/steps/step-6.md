...
- **FINALIZE 진입 안내**(마지막 IMPL일 때만) — 위 판정에서 현 PR이 스택의 마지막 IMPL이면, WRITING_REFINER 안내에 더해 FINALIZE 진입도 안내한다 ([conventions/session/finalize.md](../conventions/session/finalize.md), SKILL.md 「fan-in 후속」). 마지막이 아니면 FINALIZE 안내는 출력하지 않는다.
