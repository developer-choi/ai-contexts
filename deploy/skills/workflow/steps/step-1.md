...
`node {{skill_dir}}/scripts/step-gates.mjs artifacts --plan /plan --mode <모드>`가 이 존재 조건을 센다. 하나를 안 만들고 넘어가도 step-1.2는 그대로 굴러가고, MARKUP이 진입 문서를 못 찾는 시점에야 드러난다 — 그 시차 때문에 눈으로 세지 않는다.
...
step-1.1 종료 직후의 전환·세션경계·후속안내는 SKILL.md 「step 경계」 표가 소유한다. 위 컨벤션 인덱스 게이트를 그보다 먼저 낸다.
...
낯선 작업이라면 이 세션에서는 사용자가 방향 잡기(라이브러리 선택, 참조할 원본 코드 지정 등)에 집중하고, 구현 판단은 AI가 맡되 리뷰를 더 꼼꼼히 받는 구조로 진행한다. 그리고 공부 세션을 위한 인수인계 문서를 작성한다:
...
인수인계 문서를 쓴 직후, 다음 산출물·다음 단계로 넘어가기 전에 아래 안내를 따로 낸다. 산출물 목록이나 다른 보고 안에 한 줄로 섞지 않는다.

> 학습 인수인계 문서를 만들었습니다: `{인수인계 문서 경로}`
> 새 세션(창)을 하나 더 띄워 이 문서를 넘기고, 그 세션에서 학습을 진행해 주세요.
> 이 세션은 학습이 끝나기를 기다리지 않고 기획을 계속 진행합니다.

인수인계 문서를 작성했으면 SKILL.md 「학습 인수인계 후 세션 진입 대기」가 발동한다.
