...
자료를 읽어나가며 **확정 가능한 PR부터 하나씩** `/plan/background/consumable/project.md`에 append한다. 확정 기준·분할 원칙·의존 태깅은 [conventions/pr-split.md](../conventions/pr-split.md)가 단일 출처다.

- 모든 PR이 정해질 때까지 기다리지 않는다. 확정된 PR은 곧바로 출발 후보가 된다.
- 확정 기준은 "**다른 자료를 더 안 봐도 확정 가능한가**" — 안 읽은 자료가 그 PR의 경계를 바꿀 수 있으면 확정하지 않는다.
- 확정할 때마다 사용자에게 보고하고(범위 한 줄 + 의존 태깅 + 열린 질문), **그 자리에서** SKILL.md 「세션 spawn 안내 메커니즘」을 발동한 뒤 남은 분석을 이어간다. 확정을 모아뒀다 한꺼번에 안내하지 않는다 — 모으면 일괄 분할과 같아져 대기가 되살아난다.
