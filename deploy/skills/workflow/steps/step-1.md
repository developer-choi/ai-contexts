...
**목업 없음 + 기존 코드 존재 시 사용자 확인**: `planning`·`design`이 표 조건만으로 off 판정될 상황이라도, 리뷰 대상에 목업·시안이 없고 기존 구현 코드가 있으면(기존 동작 코드 개선형 과제 등) AI가 스스로 스킵을 판단하지 않는다. "코드를 기획·디자인 자료 대용으로 써서 planning/design 체크리스트를 리뷰할까요?"라고 사용자에게 직접 묻고, 답에 따라 플래그를 켠다. 채택 시 절차는 각 guide.md의 "코드기반 모드" 참조.
