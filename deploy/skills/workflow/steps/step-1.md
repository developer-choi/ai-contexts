...
3. **인덱스 작성**: 채택된 소스를 `/plan/background/retained/conventions-index.md`에 기록한다. 양식은 항목당 **"경로 + 한 줄 트리거(어떤 작업·디렉터리에서 봐야 하는지)"**. 본문 요약은 적지 않는다 — 본문이 바뀌면 stale해지고, 열어보면 어차피 보인다.
   - **개인·실무 모드 표준 참고처 등재**: `~/WebstormProjects/simplify/` 하위 simplified 레포(MUI·radix 등 라이브러리 원본 미러)를 표준 참고처로 인덱스에 등재한다. 트리거: "테스트·마크업·컴포넌트 작성 시 대응 컴포넌트를 이 레포들에서 뒤져 참고". 어느 레포에 어떤 컴포넌트가 있는지의 정확 매핑은 박지 않는다 — 실제 탐색은 그 자료를 소비하는 세션이 수행한다. 채용 모드는 제외.
