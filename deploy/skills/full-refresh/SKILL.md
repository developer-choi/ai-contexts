...
`<저장된 해시>..HEAD` 범위의 모든 변경 파일을 기반으로 (Phase 3 서브에이전트 수정 + 사용자 직접 커밋 포함), 각 파일의 상위 경로에 README.md가 있는지 탐색한다. 해당 README가 하위 변경을 반영하고 있는지 확인하고, 반영되지 않았으면 `/write-doc`으로 최신화한다.
