...
- 각 단계의 완료 조건이 있는지
- 서로 다른 단계가 동일한 활동이나 산출물을 중복으로 다루고 있지 않은지
...
하나의 Step 안에서 하위 넘버링(Step N.M)은 **같은 주제의 순서**여야 합니다. 서로 다른 주제의 순서(예: "작업 절차"와 "산출물 순서")를 같은 넘버링 계층에 섞지 않습니다. 다른 주제의 순서는 별도 섹션(참조, 가이드 등)으로 분리합니다.
