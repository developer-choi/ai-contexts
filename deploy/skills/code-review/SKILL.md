...
이슈를 하나 발견하면, 같은 패턴의 문제가 리뷰 대상 전체에 있는지 탐색해 함께 보고한다. 한 곳만 짚으면 사용자가 나머지를 직접 찾아 재지적하게 된다(예: 한 테스트의 동어반복 단언을 지적했다면 다른 테스트 파일에도 같은 단언이 있는지 훑는다). 탐색 범위는 리뷰 대상(작성자가 수정한 코드)에 한정한다.
