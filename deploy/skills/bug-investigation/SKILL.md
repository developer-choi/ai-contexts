...
- **Root Cause**: 왜 발생하는가. 소재를 함께 적는다 — 우리 코드 결함 / 라이브러리의 설계된 동작인데 우리 배치가 그 전제를 안 지킴 / 요구사항과 라이브러리 관행의 충돌

## 신호별 절차

- 증상이 나는 요소·스타일을 우리가 만들지 않았거나, 화면의 숫자가 우리 CSS·데이터에 없는 값이면 [library-boundary.md](library-boundary.md)를 따른다.
