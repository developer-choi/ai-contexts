...
- 무인자 `/pre-exit`에서 자동 감지: 이 세션에 `plan/pr{N}/**` 변경 존재
...
본 절차는 pre-exit 「문제 리스트업 + 규칙화」 **전에** 수행한다. 발견된 누락은 그 문제 목록에 합류한다.
...
회상 누락 보완: 본 세션의 pre-compact 스냅샷이 있으면(검출은 SKILL.md 「pre-compact 스냅샷 회수」), 그 transcript에서 Read한 `workflow/**/*.md` 항목을 추출해 보강한다.
...
회상이 압축으로 흐릿하면 pre-compact 스냅샷 transcript의 첫 사용자 항목으로 보강한다(검출은 SKILL.md 「pre-compact 스냅샷 회수」).

### 보고 vs 실제 변경 대조

세션 중 메인이 사용자에게 "X 수정함", "41번째 줄 변경", "Y 절 추가" 등으로 보고한 항목을 회상하고, 이 세션이 만든 실제 diff와 대조한다. 환각 보고(보고했지만 실제 안 변경)는 「문제 리스트업 + 규칙화」의 문제 목록에 추가한다.

### cross-ref 적합성 점검

산출물(특히 `reference.md`, `decisions.md`)이 cross-ref한 외부 문서가 실제 적합한 행선지인지, AC 「단방향 cross-ref 원칙」(`ai-contexts/CLAUDE.md`) 위반이 없는지 점검.

산출물이 인용한 내용이 링크 대상 본문에 실제로 있는지 확인하고, 양방향 결합(A→B 참조가 있을 때 B가 A를 호명)이 생겼는지 본다. 불일치·위반은 「문제 리스트업 + 규칙화」의 문제 목록에 추가한다.
...
본 보강은 별도 보고 섹션을 만들지 않는다. 발견 항목을 pre-exit 「문제 리스트업 + 규칙화」의 문제 리스트에 흡수시켜 일반 절차로 처리한다.
