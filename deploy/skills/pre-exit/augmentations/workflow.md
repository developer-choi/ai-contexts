...
세션에서 `/workflow`를 사용했거나 `plan/pr{N}/**` 산출물을 수정한 경우의 추가 회고. pre-exit 본 절차와 **함께** 수행한다 (대체 X, 추가 ○).
...
세션 중 메인이 사용자에게 "X 수정함", "41번째 줄 변경", "Y 절 추가" 등으로 보고한 항목을 회상하고 `git log -p` / `git diff`와 대조한다.
