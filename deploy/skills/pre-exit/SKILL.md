...
Step 1 **전에** 돈다. 무엇이 오래 걸렸고 지시가 어떤 순서로 왔는지를 표로 만들어 브라우저로 연다. 절차는 [timeline.md](timeline.md)를 따른다.

여기서 나온 것은 Step 1의 문제 목록에 합류한다 — 뒤에 두면 이미 만든 목록에 나중에 덧붙이게 된다. 볼 것이 없는 세션은 스크립트가 스스로 걸러낸다.
