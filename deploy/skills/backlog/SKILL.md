---
description: 스킬 관련 필기를 plan/backlog/에 쌓고, 기존 백로그를 검토·구체화·실행·정리한다. 모드 4개 — 메모 쌓기 / 리뷰(구체화·ready 태깅) / 실행([ready] 항목 실행) / 스쿼시(브랜치 정리). "백로그", "나중에", "메모해둬", "적어둬", "필기 정리", "백로그 정리", "백로그 리뷰", "ready 항목 실행", "브랜치 정리", "커밋 정리" 등 트리거. 사용자가 스킬을 지금 바로 만들거나 고치려 하면 이 스킬이 아니라 /scw를 사용한다.
...
이 스킬은 백로그 4개 모드(메모 쌓기 / 리뷰 / 실행 / 스쿼시) 전용이다. 코드·프롬프트 직접 수정은 다른 스킬로 보낸다.

| 사용자 의도 | 예시 | 행동 |
|------------|------|------|
| 나중에 하려고 메모 | "이거 백로그에 넣어", "나중에 해야지", "메모해둬" | `/backlog` 트리거 (메모 쌓기) |
| 필기·메모 정리 | "필기 정리해줘", 메모 파일 경로 제공 | `/backlog` 트리거 (메모 쌓기) |
| 백로그 검토·구체화 | "백로그 리뷰", "ready 태깅하자", "백로그 순회" | `/backlog` 트리거 (리뷰 모드) |
| ready 항목 실행 | "ready 항목 실행해줘", "백로그에서 작업 골라서 실행" | `/backlog` 트리거 (실행 모드) |
| 백로그·브랜치 정리 | "백로그 정리", "브랜치 정리", "커밋 정리" | `/backlog` 트리거 (스쿼시) |
| 지금 바로 실행 | "이거 고쳐", "바로 해줘", "프롬프트 수정해" | `/scw` 또는 직접 실행 |
| 스킬 아이디어 + 즉시 구현 | "이런 스킬 만들어줘" | `/scw` 트리거 |

핵심: **즉시 행동 의도**("~해줘", "~고쳐", "~만들어")가 백로그 외 영역(코드, 다른 스킬)이면 `/backlog`가 아니라 해당 작업을 바로 수행한다. 단 백로그 영역(정리·리뷰·실행) 내부 행동은 `/backlog` 트리거 OK.
...
백로그 항목은 **작성한 세션의 맥락 없이** 다른 세션의 AI가 읽고 실행할 수 있어야 한다. 다음을 충족해야 `[ready]`:

1. **발생한 문제**: 무엇이 잘못됐는지 사실 기반으로 서술. "참조를 빠뜨렸다"가 아니라 "Step 4 산출물의 컨벤션 참조 섹션에 coding-standards 경로가 누락됨"
2. **근본 원인**: 왜 발생했는지. 절차 누락인지, 판단 오류인지, 정보 부족인지
3. **수정 대상**: 정확한 파일 경로 (`deploy/skills/workflow/steps/step-4.md` 등)
4. **수정 내용**: 어떤 섹션에 무엇을 추가/변경하는지. 가능하면 추가할 텍스트 자체를 포함
5. **검증 방법**: 수정 후 같은 문제가 재발하지 않음을 어떻게 확인하는지 (생략 가능)

암묵적 맥락("아까 그 이슈", "방금 논의한 것")은 금지. 파일 경로, 섹션 이름, 구체적 텍스트로 지시한다.
