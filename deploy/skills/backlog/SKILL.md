...
7. **디스패치 타이밍 선택**: 승인 후 "지금 던질지, 특정 시각에 던질지" 묻는다.
   - **지금**: 8번을 즉시 실행한다.
   - **시각 지정**: 현재 로컬 시각과 지정 시각의 차이를 계산해 그 시각의 5필드 cron(`분 시 일 월 *`)을 만들고, `CronCreate(recurring: false, durable: true)`로 8번 실행을 예약한다. 재시작에도 살아남도록 각 단위 프롬프트를 backlog 레포 `orchestrate-exec/<단위슬러그>.md`에 저장하고, 예약 프롬프트엔 "이 모드의 병렬 디스패치(8번)를 실행하라 + 단위 프롬프트는 위 경로에서 읽으라"는 자족적 지시를 담는다. 시각 계산·cron 변환은 내부에서 처리하고 사용자에게 노출하지 않는다.
