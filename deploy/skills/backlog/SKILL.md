...
- `review`·`exec` 대상: 검토·실행할 항목을 좁히는 파일명·섹션 식별자(부분 일치). 생략하면 tier-1부터 전체 순회.
...
`backlog/this/`는 **할 일 트래커**(tier·Ready 게이트·리뷰/실행 모드 적용). `backlog/projects/`·`backlog/articles/`는 **비-트래커**(tier·Ready 게이트·리뷰/실행 모드 미적용) — projects/는 외부 레포 작업·지식·참고(주제별), articles/는 발행될 포스트 단위 재료 모음.
...
아래 양식은 `backlog/this/`에 적용된다 (tier·Ready 게이트 트래커). `backlog/projects/`는 경량 양식 — [projects 영역](#projects-영역)에서 정의한다. `backlog/articles/`는 [articles 영역](#articles-영역)에서 정의한다.
...
1. `backlog/this/` 하위 파일을 tier-1부터 순서대로 순회한다. 호출 인자에 대상(파일명·섹션 식별자)이 있으면 매칭 항목으로 좁힌다. `backlog/projects/`·`backlog/articles/`는 tier·Ready 게이트 미적용이라 리뷰 모드 대상이 아니다.
