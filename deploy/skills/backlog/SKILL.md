...
`UserPromptSubmit`은 매 메시지마다 발동하므로, 같은 폴더를 매번 안내하면 시끄럽다. hook은 `session_id + 폴더`를 임시 상태파일(`os.tmpdir()/claude-backlog-surfaced/<session_id>.json`)에 기록하고, **그 세션에서 그 폴더를 처음 건드릴 때 1회만** 주입한다.
