...
- **`bench`** — target이 스킬 편집이 아니라 **발생한 문제 + 벤치 재료 자체가 목적**인 ready급 항목. (「Ready 게이트」의 "행동 룰 항목이면 벤치 재료"와는 별개 축 — 그건 *스킬을 고치는 룰 항목*이 갖춰야 하는 필수 재료이고, 이 태그는 *스킬을 안 고치는 벤치 항목* 자체의 분류다)
