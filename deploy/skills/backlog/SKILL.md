...
`[ready]` 항목 전체를 의존성으로 묶어 **서로 독립적인 n개 단위**로 쪼갠 뒤, 자기 전에 사용자 1회 승인으로 **무인 병렬 디스패치**하는 상위 모드. 「실행 모드」가 단위 실행 프리미티브이고, 이 모드는 그 위에서 수집·의존성 분석·분할·프롬프트 작성·취침 전 선확인을 끝낸 뒤 단위들을 던진다.
...
7. **무인 병렬 디스패치**: 승인 후 각 단위를 백그라운드 서브에이전트로 **동시에** 던진다(단위끼리 독립이므로 순서·대기 없음). 던지고 나면 루트의 일은 끝 — 단위가 멈추거나 실패해도 종합·재시도하지 않는다.
