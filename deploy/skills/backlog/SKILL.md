...
argument-hint: "[review|exec [대상] | projects|topics|articles [메모] | 메모·파일경로]"
---
...
| `projects` / `topics` / `articles` `[메모]` | 캡처하되 [영역 분류](#1-영역-분류)를 건너뛰고 지정 영역으로 강제 라우팅. 토큰 뒤 나머지가 메모(없으면 입력 수집으로) |
...
입력된 필기를 아래 영역 중 하나로 분류한다. 하나의 필기가 여러 영역에 걸치면 각각에 배분한다. 단, 호출 인자에 영역 토큰(`projects`/`topics`/`articles`)이 주어졌으면 [인자 라우팅](#인자-라우팅)에 따라 분류를 건너뛰고 그 영역으로 직행한다.
...
대상 영역은 **`this/`(AC 자체)·`projects/`(타 프로젝트)** 뿐이다. `topics/`·`articles/`·`roadmaps/`·`history/`는 "해결할 액션"이 아니라 자료·사고 흔적이므로 표면화하지 않는다.
...
1. `backlog/this/` 하위 파일을 tier-1부터 순서대로 순회한다. 호출 인자에 대상(파일명·섹션 식별자)이 있으면 매칭 항목으로 좁힌다. `backlog/projects/`·`backlog/topics/`·`backlog/articles/`는 tier·Ready 게이트 미적용이라 리뷰 모드 대상이 아니다.
