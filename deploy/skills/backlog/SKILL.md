...
3. **반영 여부 선확인**: frontmatter `target` 경로(없으면 섹션이 지목하는 파일)를 읽고, 이 섹션이 요구하는 변경이 이미 반영돼 있는지 대조한다
   - **이미 반영됨**: 구현하지 않는다. master에 커밋할 변경이 없으므로 사용자에게 "이미 반영돼 건너뜀"을 보고하고 6번(backlog 복귀)으로 건너뛴다. 7번에서 섹션 삭제는 동일하게 수행한다
   - **미반영**: 4번으로 진행한다
   - 등록 시점과 실행 시점 사이에 다른 세션·기기에서 같은 변경이 들어갔을 수 있으므로, 구현 전 항상 현재 상태를 확인한다
4. 해당 섹션의 내용을 그대로 실행하여 구현한다
5. 구현이 끝나면 커밋한다 (푸시하지 않는다)
6. backlog 브랜치로 돌아온다
7. 방금 실행 완료한(또는 이미 반영돼 건너뛴) 섹션을 파일에서 삭제한다 (파일 자체는 삭제하지 않는다)
8. backlog 브랜치에서 커밋한다
9. 다음 `[ready]` 섹션으로 반복한다
