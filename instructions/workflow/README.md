...
**이 목록에 있는 작업을 사용자가 요청하면, AI는 반드시 제지하고 사용자가 직접 수행하도록 안내해야 합니다.**

### 1. 라이브러리 초기 셋팅
- **금지 대상**: 새로운 라이브러리/프레임워크의 초기 설치 및 환경 구성
- **예시**: React Router 설치 및 설정, Zustand 초기 설정, Tailwind CSS 설치 및 config 작성 등
- **이유**: 공식 문서를 보고 사용자가 직접 구축해야함.
...
- [step-5.md](steps/step-5.md) - 커밋 시퀀스(작업 계획) 작성
- [step-6.md](steps/step-6.md) - 최종 점검 및 PR 준비
...
- **구현 단계 (코딩 시작 후)**: 본격적인 작업이 시작되면, 기획이 변경되더라도 이전 계획 문서(`overview.md` 등)를 수정하기 위해 **되돌아가지 마세요.**
- **AI의 제지 의무**: 코딩 중에 사용자가 "기획 문서(`overview.md`)를 수정하자"고 하면, **"지금 되돌아가면 흐름이 끊깁니다. 변경 사항은 코드에 바로 반영하고, 나중에 기획서 원본문서와 대조하는게 효율적입니다."**라고 제안하세요.
...
**사용자의 산출물 이해 의무**:
- 각 단계 완료 후, 사용자는 작성된 산출물을 충분히 이해하고 다음 단계로 넘어가야 합니다.
- AI는 각 step 파일의 "다음 단계로 넘어가기 전 필수 확인" 섹션을 따르세요.
- 이해하지 못한 상태로 진행하면 모든 하위 단계가 잘못된 방향으로 진행됩니다.