...
### 모호한 부분
- "예약 확정하기" 누르면 바텀시트에서 카카오/이메일 회원가입 → 확인됨
- 수업 시작 40분 이내일 때 — 연장 버튼만 사라지는지, 영역 자체가 달라지는지?
- 타이머가 0이 되는 순간 — 바로 모달? 딜레이?

### 엣지케이스
- 수업 취소까지 남은 시간(5분 타이머): 새로고침해도 서버 기준으로 동일하게 동작해야 함 — 클라이언트 타이머가 아니라 서버 타임스탬프 기반으로 남은 시간 계산 필요
- 수업 시작까지 남은 시간: 마찬가지로 새로고침해도 정확해야 함 — 서버 시간 기준 계산
