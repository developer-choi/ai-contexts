...
1~2는 "뭐가 잘못됐나"를 찾는 과정이다. 이 단계에서는 **"뭘 추가하면 더 좋아지나"**를 생각한다. 세션에서 쌓인 맥락을 활용하여, 현재 AUDIT_SOURCE에 없지만 있으면 좋겠는 것을 제안한다.
...
- **지적 전 자가검진 추가**: 반려된 지적이 있었다면, "지적하기 전에 이걸 자문했으면 걸러졌을 텐데" 싶은 자가검진 질문을 AUDIT_SOURCE에 추가 (예: "이 지적은 문서의 의도를 고려했을 때도 유효한가?", "사용자가 이미 의도적으로 선택한 것은 아닌가?")
- **누락된 검증 단계**: "이걸 미리 체크했으면 나중에 안 고쳤을 텐데" 싶은 것이 있다면, 해당 검증을 AUDIT_SOURCE에 추가
- **도구/자동화 기회**: 수작업으로 한 것 중 스크립트나 도구로 대체할 수 있는 것
- **새로운 AUDIT_SOURCE 단계**: 기존 step 사이에 빠진 단계가 있다면 추가 제안
- **기존 단계의 세분화**: 하나의 step이 너무 많은 일을 하고 있다면 분리 제안
- **AUDIT_SOURCE 자체의 자가검진 누락**: 지금 사용 중인 AUDIT_SOURCE 자체를 살피며, 실수·오판을 사전에 걸러줄 자가검진 단계가 빠진 곳이 없는지 찾는다. (예: "지적을 출력하기 전에 한 번 더 자문하는 단계가 있는가?", "반려 패턴을 학습하는 루프가 있는가?")
