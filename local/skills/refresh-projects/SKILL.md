...
- **변경과 무관하게 낡는 것을 터는 축** — 끊어진 링크, 아무도 안 가리키는 문서, 쌓인 백로그, 기한이 지난 런타임 덤프. 아무 커밋에도 안 나타나므로 증분 추적 없이 매 회차 전수로 훑고, 변경이 없는 회차에도 돈다.

## 프로젝트 레지스트리

이 표가 정본이다 — Phase 1의 스캔이 여기서 약어·경로를 읽는다. 프로젝트를 늘리거나 옮기면 여기만 고친다.
...
Phase 0~4는 변경분(diff)을 추적·처리하는 흐름이다. 이와 별개로 [백로그 정비](#백로그-정비), [백로그 반영 sweep](#백로그-반영-sweep), [만료 덤프 정리](#만료-덤프-정리)는 변경 유무·Phase 스킵과 무관하게 매 회차 무조건 수행한다.
...
AC에서 `node scripts/refresh-projects-scan.mjs --at <약어>=<워크트리> …`. Phase 0이 만든 회차 워크트리를 프로젝트마다 넘긴다 — **이 회차가 도는 곳이 거기이고, 로컬 체크아웃을 보면 Phase 0이 막으려던 상황이 여기서 그대로 재현된다.** 안 넘긴 프로젝트는 스캔이 경고로 짚는다.

프로젝트별 범위 조회·해시 유효성·짝꿍 드리프트 후보·만료 덤프가 한 번에 나온다 — Phase 2 계획을 짜려면 이 넷이 한 화면에 있어야 한다. 손으로 git을 걸어 훑지 않는다.

스크립트가 못 정하는 둘은 여기서 정한다.
...
**한쪽만 수정된 짝꿍 그룹**을 위 스캔이 후보로 낸다. edit 시점 surfacing 훅은 세션당 1회라 놓치거나 무시될 수 있는데, 이 주기 점검이 회차 단위로 그 누락을 메운다.
...
각 항목의 반영 여부는 backlog 레포 `projects/CLAUDE.md`의 「항목 반영 여부 판정」으로 가른다. 그 판정은 개별 항목을 처리하는 시점에 그 항목 하나만 보지만, 이 sweep은 같은 기준을 `projects/` 전체에 매 회차 적용한다.
...
기한이 지난 런타임 덤프를 지운다. 덤프는 어느 커밋에도 안 나타나고 그냥 자리에 남아 낡으므로, 백로그 두 절과 같은 자리에서 **매 회차 무조건** 전수로 훑는다.

이 정리는 레지스트리 프로젝트가 아닌 곳도 대상으로 한다 — 아래 표가 정본이다.

| 대상 | 경로 | 기한 |
|---|---|---|
| PP 회사분석 덤프 | `~/WebstormProjects/main/private-playground/local/contexts/recruitment/applications/{날짜}/` | 폴더명 날짜로부터 1달 |

절차:

1. 대상 레포의 master를 최신화한 **뒤** `node scripts/refresh-projects-scan.mjs --dumps`로 다시 본다. 회차 앞(Phase 1)에서 뜬 목록은 최신화 전 상태라, 그사이 다른 기기에서 들어온 덤프 폴더가 그 회차에 영영 안 오른다.
2. 목록을 사용자에게 보고하고 승인받는다. 백로그 sweep과 같은 이유로 자동 삭제하지 않는다 — 오탐이면 다시 조사해야 되살아나는 산출물이다.
3. 승인된 폴더를 통째로 지우고, 그 자리에서 커밋해 origin master까지 올린다. 남겨두면 다른 기기·세션의 체크아웃에서 그대로 되살아난다.

PP는 지원 이력을 `application-log.json`에 따로 쌓으므로 이 삭제는 이력에 영향이 없다.
