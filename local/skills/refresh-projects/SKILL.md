...
- **변경과 무관하게 낡는 것을 터는 축** — 끊어진 링크, 아무도 안 가리키는 문서, 쌓인 백로그, 안 쓰는 워크트리·브랜치. 아무 커밋에도 안 나타나므로 증분 추적 없이 매 회차 전수로 훑고, 변경이 없는 회차에도 돈다.
...
Phase 0~4는 변경분(diff)을 추적·처리하는 흐름이다. 이와 별개로 [백로그 정비](#백로그-정비), [백로그 반영 sweep](#백로그-반영-sweep), [안 쓰는 워크트리·브랜치 정리](#안-쓰는-워크트리브랜치-정리)는 변경 유무·Phase 스킵과 무관하게 매 회차 무조건 수행한다.
...
프로젝트별 범위 조회·해시 유효성·짝꿍 드리프트 후보가 한 번에 나온다 — Phase 2 계획을 짜려면 이 셋이 한 화면에 있어야 한다. 손으로 git을 걸어 훑지 않는다.
...
워크트리 폴더와 그 브랜치는 세션이 끝나도 남는데, 어느 커밋에도 안 나타나서 다른 절차가 다시 보지 않는다. 세션 마무리의 워크트리 정리는 그 세션이 만든 것만 보므로, 정리를 건너뛰었거나 미커밋·미머지로 멈춘 채 끝난 워크트리가 여기로 온다.

diff 기반 Phase와 별개로 **매 회차 무조건 수행한다**. 회차의 **맨 끝**, [state.json 업데이트](#statejson-업데이트) 커밋과 [위임 플로우](#위임-플로우)의 dispatch 산출물 정리가 끝난 뒤에 돈다 — 그래야 이 회차가 만든 워크트리도 후보에 오른다.

### 대상

아래 루트 바로 아래의 git 레포 전부다. 워크트리는 레지스트리 밖(채용 과제·실험 레포)에도 쌓이므로 레지스트리 표로 좁히지 않는다. 스캔이 이 표를 읽는다.
...
작업이 끝난 워크트리는 회차 끝의 [안 쓰는 워크트리·브랜치 정리](#안-쓰는-워크트리브랜치-정리) 표에 올라 사용자가 지울지 고른다.
