...
로컬 체크아웃에서 돌지 않는 이유는 둘이다. 로컬이 원격보다 뒤처져 있으면 다른 기기에서 쌓인 변경이 diff 범위에서 통째로 빠져, 처리했다고 기록한 해시가 실은 안 본 지점이 된다. 그리고 사용자가 메인에서 하던 작업물이 회차 커밋에 섞인다. 워크트리는 둘 다 만들지 않는다.
...
각 작업은 `/backlog`의 「1. 영역 분류」·「3. 기존 백로그와 병합」과 backlog 레포 `CLAUDE.md`의 「영역별 경로·파일명」을 재사용한다.
