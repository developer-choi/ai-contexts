...
- 각 레포의 `local/skills`와 `CLAUDE.md`를 원본으로 보고, `.agents/skills`와 `AGENTS.md`/`GEMINI.md`를 생성/갱신한다.
- 로컬 스킬 수정이 필요하면 `.agents/`나 배포된 지시문 파일을 직접 수정하지 말고 해당 레포의 `local/skills/` 및 `CLAUDE.md`를 수정한 뒤 다시 배포한다.
