...
- 텍스트는 전량 산출물로 옮긴다.
- 텍스트 export로 안 담기는 **시각 정보**(이미지·배경색 강조·도표 등)가 있으면, 원본을 PDF 등으로 export해 눈으로 확인하고 산출물에 옮긴다 (이미지는 레포에 저장, 강조 영역은 전사). 자동으로 못 옮기는 것만 **"삭제 전 수동 캡처 필요"로 사용자에게 플래그**한다.

보고 시 원본별로 **삭제 가능 / 수동 캡처 후 삭제**를 구분해 안내한다. 파일 크기가 텍스트 분량에 비해 크면 시각 정보 임베드 신호이므로 export로 점검한다.
