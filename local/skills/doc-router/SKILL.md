...
1. Drive MCP로 폴더 바로 아래 파일 목록 조회 (하위 폴더 제외 — 폴더 직속 파일만 대상). 단, 사용자가 "전부·모든 문서" 류로 범위를 명시하면 하위 폴더 트리 전체를 순회한다
2. Drive MCP로 각 파일 내용 읽기. 이 경로는 서식을 버리므로 여기서 끝내지 않는다 — 서식·이미지 판정은 zip export HTML로 한다 (「시각 정보 캡처」)
...
원본의 그룹 헤딩은 항목의 묶음 표시일 뿐 행선지 판정 근거가 아니다. 토픽은 그룹이 아니라 항목 단위로 실제 주제를 확인해 정한다. 원본 그룹을 보존하려면 토픽이 같은 항목끼리만 보존한다.
...
- `.discarded.md` = gotchas 해당(디버깅 감정·낙서 등) **AI가 폐기 의도로 분리한** 항목 (폐기)
...
이 스킬의 목적은 구글 문서(inactive 필기 보관소)를 **비우는 것**이다. 라우팅이 끝나면 원본이 **삭제 가능한 상태**가 되는 게 종착점이며, 원본을 영구 백업으로 삼지 않는다("영구 삭제 안 함·복구 가능"은 라우팅 도중의 임시 안전망일 뿐이다). 따라서 캡처는 완전해야 한다:

- 텍스트는 전량 산출물로 옮긴다.
- 텍스트 export로 안 담기는 **시각 정보**(이미지·취소선·배경색 강조·도표 등)는 「시각 정보 캡처」 절차로 AI가 직접 옮긴다. 문서 크기가 텍스트 분량에 비해 크면 이미지 임베드 신호지만, 서식은 크기에 안 나타나므로 크기와 무관하게 문서마다 점검한다.

보고 시 원본별로 **삭제 가능 / 수동 캡처 후 삭제**를 구분해 안내한다.

### 시각 정보 캡처 (이미지·서식)

- **서식도 시각 정보다.** Drive MCP 텍스트 읽기는 취소선·형광펜을 구조적으로 버리므로, 문서마다 zip export HTML에서 `line-through`(취소선 = 이미 읽음·완료 표시)와 `background-color`(형광펜 = 사용자가 남긴 강조)를 검색해 어느 항목에 걸렸는지 확인하고 산출물에 그 표시를 보존한다. 본문 글자가 멀쩡해 보인다는 이유로 건너뛰지 않는다 — 텍스트 경로에서는 애초에 안 보인다.
- **zip export로 자동 추출한다**: Chrome 브라우저 도구로 `https://docs.google.com/document/d/<문서ID>/export?format=zip`에 접근하면 HTML+images 묶음이 브라우저 다운로드 폴더에 떨어진다. 압축을 풀고 HTML의 `<img>` 위치를 텍스트와 대조해 본문 내 이미지 자리를 매핑한다. 사용자에게 수동 다운로드·캡처를 시키지 않는다 — "삭제 전 수동 캡처 필요" 플래그는 이 경로까지 실패했을 때의 최후 수단이다.
- **Chrome zip export가 안 되면(확장 미연결 등) Drive MCP base64 export로 폴백한다.** 받은 base64·대용량은 하네스가 저장한 tool-result 파일이나 직접 저장한 디스크 경로에 두고 스크립트로 압축 해제·처리한다. **한 번 받은 base64를 도구 인자에 재-붙여넣지 않는다** — 인라인으로 온 작은 것도 마찬가지. 대화 맥락에 대용량을 다시 들이면 이후 모든 턴이 느려진다.
- **이미지는 반드시 열어본 뒤 선별한다** (무지성 전량 삽입 금지). 열람은 한 번에 전량 Read하지 말고 **문서 단위로 나눠** 연다 (한 턴의 맥락 부담 제한):
  - 텍스트로 표현 가능한 것 (코드 캡처·표·콘솔 출력·텍스트 리스트) → 코드블록·마크다운 표로 전사하고 이미지는 저장하지 않는다 (이미지 속 코드·표는 검색·AI 활용이 안 되므로)
  - 시각 정보가 본질인 것 (UI 화면·다이어그램·강조 표시) → 산출물 옆 `images/<문서슬러그>/`에 저장하고 원문 위치에 삽입한다

### 라우팅 후 역참조 갱신

원본이 삭제되면, destination 레포 안에서 그 원본을 Drive URL로 가리키던 다른 문서의 링크가 끊긴다. 완전 캡처(삭제 가능)를 유지하려면, 라우팅 직후 **방금 옮긴 원본을 가리키던 참조를 산출물의 로컬 경로로 바꾼다.**

- 방금 라우팅한 각 원본의 Drive 식별자로 destination 레포 전역을 검색한다.
- 참조하던 링크의 Drive URL을 산출물 파일의 상대경로로 치환한다.
- 단, 산출물 자신이 원본을 가리키는 **출처 표기 줄은 건드리지 않는다** — 곧 삭제될 원본을 의도적으로 남기는 출처이며, 어느 줄이 출처 표기인지는 destination 양식이 규정한다.
- 상대경로 계산·출처 줄 식별 등 치환 규약은 destination 양식 가이드·`/backlog`에 위임한다. 아직 안 옮긴(미캡처) 원본을 가리키는 링크는 그대로 둔다.
...
### 양식의 빈칸을 추론으로 채우지 않는다

라우팅은 이동이지 편집이 아니다. 양식이 요구하는 칸이 원문에 비어 있으면 추론으로 메우지 않는다.

- 원본이 맨 URL이면 맨 URL로 옮긴다. 제목을 붙이려면 실제 페이지를 열어 확인한 뒤에만 붙이고, 확인 못 하면 붙이지 않는다 — 주소 슬러그에서 유추하지 않는다.
- 사용자가 직접 적은 제목 문자열은 자르거나 사이트명을 덧붙이지 않는다.
- 진척도·종료 조건처럼 원문에서 문장을 짚을 수 없는 줄은 넣지 않는다.

지어낸 제목은 문법상 자연스러워 나중에 의심받지 않고, 원본을 지우고 나면 대조할 1차 소스가 사라져 되돌릴 수 없다.

**before** — 슬러그에서 유추한 제목:
```
[테스트 코드 접근성 개선기](https://techblog.woowahan.com/22127/) — 이거랑 MSW랑 다른 건가?
```

**after** — 페이지를 열어 확인한 제목:
```
[타입 안전한 API 모킹으로 프론트엔드 생산성 높이기 | 배민](https://techblog.woowahan.com/22127/) — 이거랑 MSW랑 다른 건가?
```

### 발췌 인용의 전량·부재 주장

필기는 외부 문서의 일부만 발췌한 것일 수 있다. 산출물에 "나머지 N건"·"~에는 없다" 같은 전량·부재 주장을 쓰려면 원본 URL을 열어 전체 항목 수를 확인한다. 확인 못 하면 개수를 세지 말고 인용한 항목만 지칭한다.

## 원본 대비 양방향 대조

산출물 작성 직후, 리뷰어를 붙이기 전에 직접 양방향으로 대조한다. 한쪽만 보면 누락과 첨가가 동시에 남는다.

- 원본 → 산출물: 링크·메모·서식(취소선·형광펜)·라벨이 전부 옮겨졌는가. 링크는 개수를 세어 맞춘다.
- 산출물 → 원본: 산출물의 각 줄을 원본에서 짚을 수 있는가. 못 짚는 줄은 AI가 첨가한 것이다.
...
API 응답 캡처 등 실명·연락처·CI·토큰 실데이터가 담긴 원시 샘플은 보류로 넘기지 않고, **값만 쓰레기 값으로 치환하고 구조·데이터 타입·문자열 양식(길이·포맷)은 보존**하여 적재한다. 샘플의 가치는 응답 형태(shape)에 있지 실값에 있지 않다. 정책성 값(응답 코드·scope·만료시간·플래그)은 원본 유지. 「gotchas」의 고유명사 원문 유지 룰과 구분한다 — 프로젝트명·회사명은 그대로 두고, 개인 식별값·시크릿만 치환한다.
...
- 디버깅 감정·소회. 열거된 예시("당황", "ㅋㅋ", "ㅠ", "ㄷㄷ", "해맸음", "구라쳐서")는 표본이며, 정보를 담지 않은 감상 표현이면 목록에 없어도 대상이다
  - 인용문 안이든 본인 문장이든 동일하게 적용한다 (출처가 GPT·공식문서여도 감상 문장은 뺀다)
  - 정보가 있는 문장에 감상이 꼬리로 붙어 있으면 문장을 살리고 꼬리만 뗀다 — 문장째 폐기하지 않는다. 뗀 꼬리는 `.discarded.md`에 항목으로 남기지 않는다 (한두 단어라 복구 가치가 없다)
- 성과·목표식 이력서 문구 ("~를 구축함", 수치 개선 자랑 등). 실제 수행 기록인지 앞으로 쓰고 싶은 샘플인지 원본으로 갈리지 않으면 보류가 아니라 폐기로 보낸다

라우팅된 항목의 고유명사(프로젝트명·repo명·계정명 등)는 익명화·일반화하지 않고 원문 그대로 둔다.
