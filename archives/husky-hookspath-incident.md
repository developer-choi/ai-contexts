...
raw worktree에서는 `npm ci` 후 `npm run prepare`를 실행하거나, AC의 `git wt-add` alias를 사용한다. AC에는 `npm run verify:hooks`를 추가해 `core.hooksPath`, `.husky/_/commit-msg`, `commitlint` 실행 파일을 커밋 전에 확인한다.
