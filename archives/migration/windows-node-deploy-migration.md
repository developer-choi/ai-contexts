...
2026-05-13 Codex가 Windows 환경에서 `npm run sync:system`을 한 번에 성공시키지 못했다. 표준 배포 명령은 하나였지만 실제 실행 경로는 PowerShell, npm shim, bash, WSL, Git 줄바꿈 설정의 영향을 동시에 받았다.

실패 흐름은 다음과 같았다.

1. PowerShell 실행 정책 때문에 `npm.ps1` 호출이 막혔다.
2. `npm.cmd run sync:system`으로 우회했지만, shell 기반 시스템 동기화 스크립트의 CRLF 줄바꿈 때문에 bash 실행이 실패했다.
...
문제의 핵심은 사용자가 어떤 셸을 쓰는지가 아니었다. `npm run sync:system`이라는 표준 명령이 내부에서 `.sh` 실행과 줄바꿈 상태에 의존하고 있었고, Windows에서 Codex가 그 의존성을 안정적으로 진단하기 어려웠다는 점이다.

## 결정

배포의 진입점은 Node로 전환한다.

- `npm run sync:system`은 `node scripts/sync-system.js`를 실행한다.
- `npm run unsync:system`은 `node scripts/unsync-system.js`를 실행한다.
- 기존 shell 기반 시스템 동기화/제거 스크립트는 제거한다. 표준 진입점은 `npm run sync:system`과 `npm run unsync:system`이며, 내부에서는 Node 스크립트만 호출한다.
...
- unsync 시 `settings.json`은 통째 삭제하지 않고 deploy가 관리하는 top-level 키만 제거한다.
- 과거 전역 agent 배포 산출물인 `agents/`는 sync/unsync 과정에서 제거한다. 전역 작업자 지시문은 별도 `deploy/agents`로 배포하지 않고 필요한 스킬 본문에 내장한다.
...
2026-05-17 Codex 마이그레이션 브랜치에서 `npm run sync:system`을 실행했을 때 Claude/Codex 파일 복사와 검증은 통과했지만, Codex hook trust 갱신 단계에서 `spawn EPERM`이 발생했다.
...
- `npm run sync:system` 재실행 결과 Codex 전역 자산 배포, `TRUST hooks.state`, Codex 검증이 모두 통과했다.
- `npm run sync:local-system -- <현재 워크트리>`로 로컬 `.claude/skills` → `.agents/skills`, `CLAUDE.md` → `AGENTS.md` 배포도 통과했다.
