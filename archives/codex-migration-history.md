...
- `npm run update`와 `npm run uninstall`을 shell script 기반에서 Node 엔트리포인트로 전환했다.
- Windows/PowerShell/CRLF/WSL 경로 차이 때문에 표준 배포 명령이 실패하던 문제를 제거했다.
- `.gitattributes`와 `.editorconfig`로 텍스트 파일의 UTF-8/LF 정책을 고정했다.
- `scripts/update.js`가 Claude와 Codex 전역 자산을 함께 배포하고 검증한다.
- `scripts/uninstall.js`가 update와 같은 기준으로 Claude/Codex 전역 자산을 제거한다.
- `npm run deploy-local-skills`를 추가해 각 레포의 `.claude/skills`를 `.agents/skills`로, 루트 `CLAUDE.md`를 루트 `AGENTS.md`로 배포한다.
- `repo-sync` 후속 단계에서 `npm run update`와 `npm run deploy-local-skills`를 모두 실행하도록 정리했다.
...
- `~/.codex/skills/.system`은 Codex 런타임 소유이므로 update/uninstall 대상에서 제외한다.
...
- Codex non-managed hook은 trusted 상태가 아니면 실행되지 않으므로, 신뢰 갱신까지 update 검증 범위에 포함했다.
...
- 과거 전역 agent 배포 산출물인 `agents/`는 update/uninstall 과정에서 제거한다.
...
- `npm run update`가 Claude/Codex 전역 자산 배포, 검증, Codex hook trust 갱신까지 통과했다.
- `npm run deploy-local-skills -- <worktree>`가 `.claude/skills`에서 `.agents/skills`로, `CLAUDE.md`에서 `AGENTS.md`로 배포를 통과했다.
...
- Windows에서 `npm run update`가 PowerShell 실행 정책, npm shim, CRLF bash 실행, WSL 경로 변환에 연쇄적으로 걸려 실패했다. 배포 진입점을 Node로 전환해 해결했다.
...
- 시스템이 소유하는 디렉터리나 파일은 update/uninstall에서 보존한다.
- update가 새 대상을 배포하면 uninstall도 같은 기준으로 제거하게 만든다.
