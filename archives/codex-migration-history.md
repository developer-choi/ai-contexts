...
- `npm run sync:local-skills`를 추가해 각 레포의 `.claude/skills`를 `.agents/skills`로, 루트 `CLAUDE.md`를 루트 `AGENTS.md`로 배포한다.
- `repo-sync` 후속 단계에서 `npm run sync:system`과 `npm run sync:local-skills`를 모두 실행하도록 정리했다.
...
- `npm run sync:local-skills -- <worktree>`가 `.claude/skills`에서 `.agents/skills`로, `CLAUDE.md`에서 `AGENTS.md`로 배포를 통과했다.
