# Step 1: 핵심 과제 정의 및 기술 전략 수립
이 단계는 **하나의 PR**에 대한 핵심 기술적 과제를 정의하고, 최적의 도구(라이브러리)를 선정하는 단계입니다.

기획/디자인 부재 시 전반적인 UI/UX 컨셉도 확정하는 단계입니다.

## [CRITICAL] Step 0을 먼저 진행했나요?

다음 중 하나라도 해당되면 **Step 1이 아닌 [Step 0](./step-0.md)부터 진행**하세요:
...
- 각 PR별로 Step 1을 반복하며 `/plan/overview.md` 작성
...
1. **먼저 Step 0로 유도**:
   - "이 작업은 여러 PR로 나누는 것이 좋겠습니다. 먼저 Step 0에서 전체 범위와 PR 분할 전략을 정리할까요?"

2. **Step 0 완료 후**:
   - "Step 0에서 정리한 PR #1부터 시작하겠습니다. Step 1으로 진행할게요."
...
## Step 1-A: background.md 작성 (최초 1회)

**background.md가 이미 있으면 건너뛰고 Step 1-B로 이동하세요.**
...
## Step 1-B: Step 0 진행 여부 판단

background.md 작성 후, 다음 중 하나라도 해당되면 **[Step 0](./step-0.md)으로 이동**하세요:

- ☐ 여러 PR로 나눠야 하는 큰 프로젝트
- ☐ 3개 이상의 페이지/컴포넌트가 영향을 받음
- ☐ 공통 모듈 개발이 필요함
- ☐ 외부 협업(앱팀, 백엔드팀)이 필요함
- ☐ 단계적 배포가 필요함

**해당사항 없으면 Step 1-C로 진행하세요.**

---

## Step 1-C: overview.md 작성

[roadmap.md](../self-help/roadmap.md)의 사고 흐름을 적용하여 `/plan/overview.md`를 작성하세요.
