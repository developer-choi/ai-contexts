...
**각 단계를 완료할 때마다, 반드시 사용자에게 의견을 물어보고 승인을 받은 후에만 다음 단계로 진행하세요.**
...
- "overview.md를 완성했습니다. Step 4는..." (승인 안 받고 진행)
...
- **단계별 승인 대기**: 각 단계 완료 후 **반드시 사용자 승인을 받고** 다음 단계로 진행
- **역질문**: 정보 부족 시 사용자에게 질문
- **누락 방지**: 매 단계 최종 검토 필수
- **[CRITICAL] 역주행 금지 (구현 단계 진입 후 동기화 지양)**:
...
- **[CRITICAL] 이전 산출물 계승 (Context Loading)**:
  - **읽고 시작하세요**: 각 단계를 시작하기 전, 반드시 이전 단계들에서 생성된 산출물(`background.md`, `codebase-audit.md`, `overview.md` 등)을 읽어 내용을 숙지해야 합니다.
  - **맥락 반영**: 분석된 내용(제약 사항, 기획 의도 등)을 현재 작성 중인 문서에 녹여내야 합니다.