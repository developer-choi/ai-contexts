...
- 파일 구성이 바뀐 주제는 `backlog/projects/{project}/{topic}/index.md`를 갱신한다.
...
현재 프로젝트에서 이 회차에 추가·수정된 스킬을 점검한다. 대상은 그 프로젝트의 스킬 — AC는 전역(`deploy/skills/`)+로컬(`.claude/skills/`), KA는 로컬(`.claude/skills/`).
