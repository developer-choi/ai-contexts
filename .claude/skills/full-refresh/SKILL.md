...
레지스트리의 Phase 5 스킬이 등록된 프로젝트별로 팀 에이전트를 위임한다. KA 기반 변환이므로 Phase 3의 KA Maintain 완료 후 실행한다.

팀 에이전트 지시는 Phase 3과 동일한 원칙을 따른다. 특히 **규칙 변경 감지**: Deploy 스킬의 변환 규칙(예: Blog CLAUDE.md의 KA 변환 규칙, KQ parse 스크립트)이 변경되었으면 scope를 "전체 재생성"으로 확장한다.
...
팀 에이전트는 **서브에이전트를 만들지 못한다**는 제약이 있어, 대량 파일 변환·생성 작업에서 병렬화가 불가능하다. 이 경우 아래 위임 플로우를 사용한다.

### 대규모 판정 기준

Phase 3/5 대상 파일이 10개 이상이거나 Blog Deploy처럼 한 세션에서 수행 시 현저히 느려질 변환 작업.
...
2. **사용자 안내**: 다음 형식으로 전달한다.
   > 다음 md를 새 Claude Code 세션에 복사해 실행해 주세요. 새 세션은 메인 에이전트이므로 서브에이전트를 병렬 spawn하여 대량 작업을 빠르게 처리할 수 있습니다. 완료 후 `-result.md` 경로를 알려주시면 이어서 진행합니다.
...
소규모 작업(단일 파일 수정, Maintain 일부)은 기존대로 팀 에이전트 유지.
