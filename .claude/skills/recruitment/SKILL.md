...
| "내일 OOO 기술면접" | technical 모드 진입. **`company-analysis/` 호출하여 회사 분석 산출물 생성/갱신** → 키워드 추출 → KA 매칭 + 예상질문/역질문/자기소개 합성. 베이스: `technical-interview/` 전체 + `steps/prep.md` | 그 회사용 준비물.md (1파일) |
| "내일 OOO 컬쳐핏/임원" | culture-fit 모드 진입. **`company-analysis/` 호출하여 회사 분석 산출물 생성/갱신** → behavioral Q&A + 대표 성향별 역질문 + 자기소개 합성. 베이스: `culture-fit-interview/` 전체 + `steps/prep.md` | 그 회사용 준비물.md (1파일) |
