...
.claude/skills/recruitment/
  ...
  company-analysis/                     # 회사 분석 sub-skill (양 모드 공유)
    SKILL.md
    output-template.md
  steps/                                # 모드 무관 공통 흐름 (Step 2~5)
    prep.md
    entry.md
    retrospective.md
  technical-interview/                  # 기술면접 모드 (베이스 파일만)
  ...
  culture-fit-interview/                # 컬쳐핏 + 임원 통합 모드 (베이스 파일만)
...
**AI 액션** — 각 단계는 외부 리서치 동반 (1년 만에 변한 트렌드를 그때그때 끌어와 반영). AI가 아래 항목을 위에서부터 진행 관리한다. 항목별 사용자 컨펌 후 다음으로 넘어간다.

- recruitment 전체 베이스 파일 훑기 — 1년치 사용자 성장 반영. AI가 사용자와 인터뷰하며 추가/수정/제거할 부분 찾기. 대상은 `steps/`, `company-analysis/`, `technical-interview/`, `culture-fit-interview/`, `tips/` 하위 모든 베이스 파일.
...
| "내일 OOO 기술면접" | technical 모드 + culture-fit 모드 **동시** 진입 (기술면접에서도 컬쳐핏 질문이 섞여 나오므로). **`company-analysis/` 호출하여 회사 분석 산출물 생성/갱신** → 공통 `steps/prep.md` 흐름 진행. 베이스: `technical-interview/` + `culture-fit-interview/` 양쪽 베이스 파일 | 그 회사용 준비물.md (1파일) |
| "내일 OOO 컬쳐핏/임원" | culture-fit 모드 **단독** 진입 (컬쳐핏만 더 깊게). **`company-analysis/` 호출하여 회사 분석 산출물 생성/갱신** → 공통 `steps/prep.md` 흐름 진행. 베이스: `culture-fit-interview/` 베이스 파일 | 그 회사용 준비물.md (1파일) |
| "면접 입장 직전" | 공통 `steps/entry.md` 진입. 직전 카드 훑기 + 태도 리마인드 | 짧은 카드 |

### 시점 3: 면접 직후 회고

**트리거**: "OOO 면접 끝났어" / "면접 회고하자".

**AI 액션**: 회고 인터뷰를 진행해 `recruitment/` 하위 적절한 md를 다듬는다. 어디에 뭘 반영할지는 AI 판단. 베이스: 공통 `steps/retrospective.md`.

**산출물**: 다듬어진 베이스 파일들 (다음 면접에 자동 반영됨).

## 모드 진입 규칙

흐름(prep/entry/retrospective)은 모드 무관 공통이며, 모드는 **어떤 베이스 질문 파일을 사용할지**만 결정한다.

- **technical 모드** (기술면접 트리거): `technical-interview/` + `culture-fit-interview/` 양 폴더 베이스 파일 동시 로드 + 공통 `steps/`. 기술면접에서도 컬쳐핏 질문이 섞여 나오므로 함께 준비.
- **culture-fit 모드** (컬쳐핏/임원 트리거): `culture-fit-interview/` 베이스 파일 단독 로드 + 공통 `steps/`. 컬쳐핏 단독 면접에서는 더 깊게 다룬다. 컬쳐핏과 임원면접은 같은 모드로 통합 처리.
