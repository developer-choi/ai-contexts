...
구글 문서의 혼합 콘텐츠를 입력 폴더에 통합 MD로 합치고, 명백한 항목만 KA / MP patterns로 라우팅한다. 잔여는 통합 MD에 라벨링하여 사용자에게 넘긴다 — **AI는 자동으로 버리지 않는다.** KA·MP 최종 양식 변환은 각 프로젝트에서 별도로 수행한다. **글의 행선지만 결정**한다 — 어떤 스킬이 어떻게 활용할지(`/exam` 대상 등)는 결정하지 않으며, 글에 활용 라벨을 박지 않는다.
...
- 파일명: `<원본명>.merged.md` (예: `State.pdf` + `State.md` → `State.merged.md`)
...
통합 MD는 분류·라우팅 작업의 작업 공간이자 최종 잔여 보관소다 (산출물 섹션 참조).

## 분류 진입 조건

통합 MD에 본문 텍스트 + URL + 이미지 텍스트화가 모두 들어있어야 분류로 진입한다. 헤딩만 잡힌 상태, MD 본문이 누락된 상태, PDF 이미지가 옮겨지지 않은 상태에서는 분류 단위 판단·비중 가드 보고·라우팅으로 넘어가지 않는다.
...
의심스러우면 옮기지 않는다. 통합 MD에 `[BORDERLINE]` 라벨을 달고 잔여에 남긴다. 최종 판단은 사용자가 한다.
...
AI는 자동으로 버리지 않는다. 행선지가 명확하지 않거나 gotchas에 해당하는 항목은 통합 MD에 라벨링하여 잔여로 남긴다. 옮긴 항목은 통합 MD에서 본문을 삭제한다 (이동 표시도 남기지 않음 — 원본 PDF/MD가 보존되므로 복구 가능).

| 라벨 | 의미 |
|------|------|
| `[NOISE_CANDIDATE]` | gotchas 해당 — 디버깅 감정·낙서·개인 메모·고유명사 등 버림 권장 |
| `[BORDERLINE]` | 행선지 애매하거나 본인 통합 흔적 약함 |

라벨 없는 잔여 = 일반 잔여 (사용자 판단 필요).

잔여 라벨은 섹션 헤딩 옆에 단다. 사유 한 줄을 함께 적는다.
...
사용자가 판단한 결과를 알려주면 즉시 gotchas 섹션에 기록한다.
...
- **TODO·References 항목** → AC `plan/topics/`로 모은다. 양식·절차는 [`/backlog` 스킬](../../../deploy/skills/backlog/SKILL.md)에 위임한다.
- **명백히 무가치해 보이는 항목** (의미 없는 낙서, 중복 등) → 옮기지 않고 통합 MD 잔여에 `[NOISE_CANDIDATE]` 라벨링. 최종 판단은 사용자.
...
- **통합 MD (잔여 포함)** — 입력 폴더의 `<원본명>.merged.md`. 옮긴 섹션은 본문 삭제, 잔여만 라벨링과 함께 남김

사용자에게는 "<입력 폴더 경로>의 `.merged.md` 파일들 확인" 한 줄로 보고한다. 잔여를 추가로 정리하지 않는다 — 사용자가 직접 본문 삭제·이동 지시한다.
...
다음 항목은 통합 MD 잔여에 `[NOISE_CANDIDATE]` 라벨링. AI는 자동 삭제하지 않는다.
